/**
 * End-to-end tests for event-sourced registry with the full Koi runtime.
 *
 * Validates the event-sourced AgentRegistry through createKoi (L1) + createPiAdapter
 * with real LLM calls. Exercises:
 *   - Agent registration in event-sourced registry
...
describeE2E(
  "e2e: event-sourced registry with real Anthropic API via createKoi + createPiAdapter",
  () => {
    // ── Test 1: Full lifecycle — register, run, verify events ──────────────

    test(
      "agent completes real LLM call with event-sourced registry tracking lifecycle",
      async () => {
        // Create event-sourced registry with in-memory backend
        const backend = createInMemoryEventBackend();
        const registry = await createEventSourcedRegistry(backend);

        // Register the agent in the event-sourced registry
    ...
    test(
      "middleware hooks fire during real LLM call with event-sourced registry",
      async () => {
        const backend = createInMemoryEventBackend();
        const registry = await createEventSourcedRegistry(backend);
    ...
    test(
      "real LLM metrics are non-zero with event-sourced registry tracking full lifecycle",
      async () => {
        const backend = createInMemoryEventBackend();
        const registry = await createEventSourcedRegistry(backend);
