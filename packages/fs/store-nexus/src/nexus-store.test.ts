/**
 * Tests for the Nexus-backed ForgeStore implementation.
 *
...
import { createNexusForgeStore } from "./nexus-store.js";
...
describe("Nexus-specific", () => {
  ...
  test("bounded concurrency limits parallel reads", async () => {
    ...
    // Search triggers reads of all 9 in batches of 3
  ...
  test("watch fires for local mutations only", async () => {
  ...
  test("dispose clears change listeners", async () => {
    const store = createStore();
    if (store.watch === undefined || store.dispose === undefined) {
      return;
    }
    ...
    expect(events).toHaveLength(1); // unchanged after dispose
  });
});
