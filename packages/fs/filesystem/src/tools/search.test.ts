...
describe("createFsSearchTool", () => {
  test("returns search result on success", async () => {
    const tool = createFsSearchTool(createMockBackend(), "fs", "verified");
  ...
  test("passes options to backend", async () => {
    ...
    const tool = createFsSearchTool(backend, "fs", "verified");
    await tool.execute({
      pattern: "hello",
      glob: "**/*.ts",
      maxResults: 100,
  ...
  test("returns error object on backend failure", async () => {
    const tool = createFsSearchTool(createFailingBackend(), "fs", "verified");
  ...
  test("descriptor has correct name and required fields", () => {
    const tool = createFsSearchTool(createMockBackend(), "rg", "sandbox");
