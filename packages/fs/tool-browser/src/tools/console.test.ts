...
describe("browser_console", () => {
  test("returns console entries on success", async () => {
    ...
    const tool = createBrowserConsoleTool(driver, "browser", DEFAULT_UNSANDBOXED_POLICY);
  ...
  test("uses custom prefix in tool name", () => {
    const driver = createMockDriver();
    const tool = createBrowserConsoleTool(driver, "web", DEFAULT_UNSANDBOXED_POLICY);
