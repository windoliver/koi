...
/**
 * Symbol stamped on allow decisions produced by exact-string rules (no glob
 * metacharacters). When set, hasExplicitExactArgvRule trusts the allow directly
 * without a canary probe — the canary heuristic fails when both an exact rule
 * and a broader allow (`bash:*`) are present simultaneously.
 */
const IS_EXACT_MATCH: symbol = Symbol.for("@koi/permissions/exact-match");
...
/**
 * Symbol stamped on ask decisions generated by the spec guard.
 * Shared via Symbol.for() so handleAskDecision can disable legacy bash grant
 * fallback for these asks without a cross-L2 import.
 *
 * Legacy fallback logic checks the resource string shape (e.g. `!complex` prefix),
 * but after spec-guard rebinding the resource is the exact argv — the shape check
 * is no longer reliable. This marker is the authoritative signal.
 */
export const IS_SPEC_GUARD_ASK: symbol = Symbol.for("@koi/middleware-permissions/spec-guard-ask");

function specGuardAsk(reason: string): PermissionDecision {
  const d: PermissionDecision & Record<symbol, boolean> = {
    effect: "ask",
    reason,
    [IS_SPEC_GUARD_ASK]: true,
  };
  return d;
}
...
async function hasExplicitExactArgvRule(
  ...
  // If the backend stamped IS_EXACT_MATCH, the allow came from a literal rule —
  // no canary probe needed. This correctly handles the case where both an exact
  // rule and a broader allow (e.g. `bash:*`) are present: the canary would also
  // be allowed by the broad rule, producing a false negative.
  const withMeta = exactDecision as Record<symbol, unknown>;
  if (withMeta[IS_EXACT_MATCH] === true) {
    return true;
  }
  // Canary suffix fallback: a prefix/glob rule that matched the exact resource
  // will also match `${exactResource}\x01__canary__` because glob `*` compiles
  // to `[^/]*` which matches any non-slash chars including the appended bytes.
  // An exact rule compiled to `^…$` will NOT match the canary.
    ...
    if (currentDecision.effect === "allow") {
      ...
      if (!hasExplicit) {
        ...
        return {
          kind: "spec-evaluated",
          decision: specGuardAsk(
            `Spec (${specResult.kind}: ${label}); exact-argv Run(...) rule required`,
          ),
