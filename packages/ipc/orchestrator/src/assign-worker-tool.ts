...
/**
 * Creates the assign_worker executor with an encapsulated worker counter.
 */
export function createAssignWorkerExecutor(): (
  raw: unknown,
  holder: BoardHolder,
  ...
  return async function execute(
    ...
    if (spawnResult.ok) {
      ...
      const completeResult = holder.getBoard().complete(id, {
        taskId: id,
        output,
        durationMs: spawnResult.durationMs ?? durationMs,
        artifacts: spawnResult.artifacts,
        decisions: spawnResult.decisions,
        warnings: spawnResult.warnings,
        workerId,
      });
      if (completeResult.ok) {
        holder.setBoard(completeResult.value);
        return `Task ${input.task_id} completed by ${workerId}. Output: ${output.slice(0, 200)}`;
      }
