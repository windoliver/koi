/**
 * forge_resolver — Creates a new resolver brick through the verification pipeline.
 */

import type { Result, Tool } from "@koi/core";
import type { ForgeError } from "../errors.js";
import type { ForgeResolverInput, ForgeResult, ImplementationArtifact } from "../types.js";
...
const FORGE_RESOLVER_CONFIG: ForgeToolConfig = {
  name: "forge_resolver",
  description:
    "Creates a new resolver brick by running it through the 4-stage verification pipeline. " +
    "Resolvers require 'verified' trust before attachment.",
  inputSchema: {
    type: "object",
    properties: {
      name: { type: "string" },
...
async function forgeResolverHandler(
  input: unknown,
  deps: ForgeDeps,
  ...
  const forgeInput: ForgeResolverInput = {
    kind: "resolver",
...
export function createForgeResolverTool(deps: ForgeDeps): Tool {
  return createForgeTool(FORGE_RESOLVER_CONFIG, deps);
}
