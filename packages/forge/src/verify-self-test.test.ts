...
describe("verifySelfTest — SelfTestStageReport counts", () => {
  test("populates testCasesRun from input test cases", async () => {
    const executor = mockExecutor(async () => ({
      ok: true,
      value: { output: 42, durationMs: 1 },
    }));
    const input: ForgeInput = {
      ...
      implementation: "return 42;",
      testCases: [
        { name: "t1", input: {}, expectedOutput: 42 },
        { name: "t2", input: { a: 1 }, expectedOutput: 42 },
      ],
    };
    const result = await verifySelfTest(input, executor, [], DEFAULT_CONTEXT, DEFAULT_VERIFICATION);
    expect(result.ok).toBe(true);
    if (result.ok) {
      const report = result.value as SelfTestStageReport;
      expect(report.testCasesRun).toBe(2);
    }
  });

  test("uses testCounts parameter when provided", async () => {
    const input: ForgeInput = {
      kind: "tool",
      name: "myTool",
      description: "A tool",
    ...
    const result = await verifySelfTest(
      ...
      DEFAULT_CONTEXT,
      DEFAULT_VERIFICATION,
      { agentProvided: 2, autoGenerated: 1 },
    );
    expect(result.ok).toBe(true);
    if (result.ok) {
      const report = result.value as SelfTestStageReport;
      expect(report.agentProvidedCount).toBe(2);
      expect(report.autoGeneratedCount).toBe(1);
      expect(report.testCasesRun).toBe(3);
    }
  });

  test("defaults to zero autoGenerated when testCounts not provided", async () => {
    const input: ForgeInput = {
      kind: "tool",
      name: "myTool",
      description: "A tool",
    ...
    if (result.ok) {
      const report = result.value as SelfTestStageReport;
      expect(report.agentProvidedCount).toBe(1);
      expect(report.autoGeneratedCount).toBe(0);
    }
  });

  test("reports zero counts when no test cases exist", async () => {
    const input: ForgeInput = {
      kind: "tool",
      name: "myTool",
      description: "A tool",
    ...
    if (result.ok) {
      const report = result.value as SelfTestStageReport;
      expect(report.testCasesRun).toBe(0);
      expect(report.autoGeneratedCount).toBe(0);
    }
  });
});
