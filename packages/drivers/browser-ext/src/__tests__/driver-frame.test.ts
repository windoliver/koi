...
describe("isDriverOriginated / isHostOriginated", () => {
  ...
  test("admin_clear_grants is driver-originated and ack is host-originated", () => {
    expect(isDriverOriginated({ kind: "admin_clear_grants" } as never)).toBe(true);
    expect(isHostOriginated({ kind: "admin_clear_grants_ack" } as never)).toBe(true);
  });
