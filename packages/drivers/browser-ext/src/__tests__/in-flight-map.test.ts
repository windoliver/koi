...
describe("InFlightMap", () => {
  test("composite key isolates same attachRequestId across clients", () => {
    ...
    m.add({ tabId: 42, clientId: "c1", attachRequestId: "r1", receivedAt: 1, abandoned: false });
    m.add({ tabId: 42, clientId: "c2", attachRequestId: "r1", receivedAt: 2, abandoned: false });
  ...
  test("markAbandonedByClient flips only matching entries", () => {
    const m = createInFlightMap();
    m.add({ tabId: 42, clientId: "c1", attachRequestId: "r1", receivedAt: 1, abandoned: false });
    m.add({ tabId: 42, clientId: "c1", attachRequestId: "r2", receivedAt: 2, abandoned: false });
    m.add({ tabId: 43, clientId: "c2", attachRequestId: "r3", receivedAt: 3, abandoned: false });
  ...
  test("entriesForTab filters out abandoned", () => {
    const m = createInFlightMap();
    m.add({ tabId: 42, clientId: "c1", attachRequestId: "r1", receivedAt: 1, abandoned: false });
    m.add({ tabId: 42, clientId: "c2", attachRequestId: "r2", receivedAt: 2, abandoned: false });
