...
function createMockSdk(overrides?: Partial<CfSdkSandbox>): CfSdkSandbox {
  return {
...
describe("createCloudflareInstance", () => {
  test("exec runs command and returns result", async () => {
    const sdk = createMockSdk();
    const instance = createCloudflareInstance(sdk);
  ...
  test("throws after destroy", async () => {
    ...
    const instance = createCloudflareInstance(sdk);
    await instance.destroy();
    expect(() => instance.exec("cmd", [])).toThrow(
      "cloudflare: cannot call exec() after destroy()",
    );
  });
