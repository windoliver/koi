...
import { consumeSSEStream, type SSEEvent, SSEParser } from "./sse-stream.js";
...
describe("SSEParser", () => {
  ...
  test("handles line split at \\n boundary", () => {
    ...
    expect(e1).toHaveLength(0); // no empty line yet
  ...
  test("ignores keepalive (empty comment)", () => {
    ...
    // Empty line after comment — but no data lines, so no event dispatched
  ...
  test("id persists across events", () => {
    const parser = new SSEParser();
    parser.feed("id: 1\ndata: first\n\ndata: second\n\n");
    // id should still be "1" after second event (per spec)
  ...
  test("ignores id containing null character", () => {
    ...
    // Should still be "1" — the null-containing id is ignored
    expect(parser.lastId).toBe("1");
  });
  ...
  test("reset clears buffer but preserves lastId", () => {
    ...
    expect(parser.lastId).toBe("42"); // preserved
  });
...
describe("consumeSSEStream", () => {
  ...
  test("handles abort signal", async () => {
    ...
    const body = new ReadableStream<Uint8Array>({
      ...
      pull() {
        // Simulate a long-running stream that never sends more data.
        // The abort signal should cancel the pending reader.read().
