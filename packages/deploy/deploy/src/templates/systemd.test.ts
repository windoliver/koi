...
import { generateSystemdUnit, type SystemdTemplateConfig } from "./systemd.js";
...
describe("generateSystemdUnit", () => {
  ...
  it("sets WantedBy=multi-user.target for system services", () => {
    const output = generateSystemdUnit({ ...BASE_CONFIG, system: true });
  ...
  it("includes EnvironmentFile when envFile is set", () => {
    const output = generateSystemdUnit({ ...BASE_CONFIG, envFile: "/app/.env" });
  ...
  it("includes User when user is set", () => {
    const output = generateSystemdUnit({ ...BASE_CONFIG, user: "koi" });
    expect(output).toContain("User=koi");
  });

  it("handles restart: always", () => {
    const output = generateSystemdUnit({ ...BASE_CONFIG, restart: "always" });
    expect(output).toContain("Restart=always");
  });

  it("handles restart: no", () => {
    const output = generateSystemdUnit({ ...BASE_CONFIG, restart: "no" });
  ...
  it("rejects manifestPath with shell metacharacters", () => {
    expect(() =>
      generateSystemdUnit({ ...BASE_CONFIG, manifestPath: "/app/$(whoami).yaml" }),
    ).toThrow("manifestPath contains unsafe characters");
  });

  it("rejects bunPath with newlines", () => {
    expect(() =>
      generateSystemdUnit({ ...BASE_CONFIG, bunPath: "/usr/bin/bun\nExecStartPost=/bin/evil" }),
    ).toThrow("bunPath contains unsafe characters");
  });

  it("rejects name with semicolons", () => {
    expect(() => generateSystemdUnit({ ...BASE_CONFIG, name: "agent;rm -rf /" })).toThrow(
      "name contains unsafe characters",
    );
  });

  it("rejects port 0", () => {
    expect(() => generateSystemdUnit({ ...BASE_CONFIG, port: 0 })).toThrow("Invalid port");
  });

  it("rejects port 70000", () => {
    expect(() => generateSystemdUnit({ ...BASE_CONFIG, port: 70000 })).toThrow("Invalid port");
  });

  it("rejects envFile with backticks", () => {
    expect(() => generateSystemdUnit({ ...BASE_CONFIG, envFile: "/app/`whoami`.env" })).toThrow(
