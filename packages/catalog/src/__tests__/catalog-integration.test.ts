...
describe("catalog integration", () => {
  ...
  test("bundled search: filter by kind returns only matching entries", async () => {
    ...
    // Should have all 26 middleware packages
    expect(middlewarePage.items.length).toBe(26);
  ...
  test("bundled entries total count matches expected", async () => {
    ...
    // 26 middleware + 6 channel + 4 engine + 10 sandbox + 3 tool + 21 infra = 70
    expect(BUNDLED_ENTRIES.length).toBe(70);
  });
});
