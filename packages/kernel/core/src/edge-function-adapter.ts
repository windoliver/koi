...
export type EdgeDestroyOutcome =
  | {
      ...
      /** Provider artifacts that were successfully torn down (gateway + handler-runner for paired adapters). */
      readonly providerArtifacts?: readonly string[];
    }
  | {
      readonly kind: "destroyed-local-remote-leaked";
      /** Artifacts that remained allocated. Multiple entries when a paired adapter (gateway + handler-runner) failed teardown. */
      readonly providerArtifacts: readonly string[];
