...
export interface SnapshotChainStore<T> {
  ...
  /**
   * Replace the payload of an existing node without changing its ID or
   * parent pointers. Used by the "two-phase capture" pattern: the initial
   * `put` writes a snapshot synchronously on the critical path; a deferred
   * background task (drift detection, async hashing, etc.) then calls
   * `updatePayload` to fill in fields that weren't available at put time.
   *
   * The node's `contentHash` is recomputed from the new data, but `nodeId`,
   * `parentIds`, `chainId`, `createdAt`, and `metadata` all remain stable.
   * Concurrent readers observing the same nodeId will see either the old
   * or the new data atomically — never a torn view.
   *
   * Returns a `NOT_FOUND` error if the nodeId does not exist.
   */
