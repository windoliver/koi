...
const acceptAllAuth: CanvasAuthenticator = async (request): Promise<Result<CanvasAuthResult>> => {
...
describe("canvas routes", () => {
  ...
  test("GET with matching If-None-Match → 304", async () => {
    ...
    const res = await fetch(url(server, "/etag-1"), { headers: { "If-None-Match": etag } });
  ...
  test("SSE endpoint returns text/event-stream", async () => {
    ...
    const res = await fetch(url(server, "/sse-1/events"), { signal: controller.signal });
  ...
  test("SSE does not require auth", async () => {
    ...
    const controller = new AbortController();
    const res = await fetch(url(server, "/sse-pub/events"), { signal: controller.signal });
