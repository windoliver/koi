/**
 * Resolve the argv used to launch nexus.
 *
 * Priority:
 *   1. `opts.command` — explicit argv override
 *   2. `NEXUS_COMMAND` env var — space-separated argv
 *   3. `opts.sourceDir` — `uv run --directory <sourceDir> nexus` (contributor mode)
 *   4. Default: `uvx --from nexus-ai-fs nexus` (isolated, no PATH conflicts)
 *
 * Returned argv does NOT include `serve` — `lifecycle.startSandbox`
 * appends the subcommand so `resolveCommand` is testable on its own.
 */

import type { ResolveCommandOptions } from "./types.js";

const DEFAULT_ARGV: readonly string[] = Object.freeze(["uvx", "--from", "nexus-ai-fs", "nexus"]);
...
export function resolveCommand(opts: ResolveCommandOptions = {}): readonly string[] {
  ...
  if (opts.sourceDir !== undefined) {
    return Object.freeze(["uv", "run", "--directory", opts.sourceDir, "nexus"]);
  }
  return DEFAULT_ARGV;
}
