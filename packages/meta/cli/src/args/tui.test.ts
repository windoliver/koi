...
describe("parseTuiFlags — --until-pass / --max-iter (#1624)", () => {
  ...
  test("single --until-pass token is collected as 1-element argv", () => {
    const flags = parseTuiFlags(["--until-pass", "bun", "--allow-side-effects"]);
  ...
  test("--max-iter parses to integer", () => {
    const flags = parseTuiFlags(["--until-pass", "bun", "--max-iter", "5", "--allow-side-effects"]);
    expect(flags.maxIter).toBe(5);
  });

  test("--max-iter rejects strings with trailing junk (strict)", () => {
    expect(() => parseTuiFlags(["--max-iter", "10abc"])).toThrow(ParseError);
  });

  test("--max-iter rejects zero / negative", () => {
    expect(() => parseTuiFlags(["--max-iter", "0"])).toThrow(ParseError);
    expect(() => parseTuiFlags(["--max-iter", "-1"])).toThrow(ParseError);
  });
  ...
  test("empty --until-pass token is rejected", () => {
    expect(() => parseTuiFlags(["--until-pass", ""])).toThrow(ParseError);
  });

  test("--until-pass without --allow-side-effects is rejected (trust boundary guard)", () => {
    expect(() => parseTuiFlags(["--until-pass", "bun"])).toThrow(ParseError);
  });

  test("--until-pass combined with --session is rejected (silent-state-loss guard)", () => {
    expect(() =>
      parseTuiFlags(["--until-pass", "bun", "--session", "ses_abc", "--allow-side-effects"]),
    ).toThrow(ParseError);
  });

  test("--verifier-inherit-env is captured when explicitly passed", () => {
    const without = parseTuiFlags(["--until-pass", "bun", "--allow-side-effects"]);
  ...
  test("--allow-side-effects is captured on the returned flags", () => {
    const without = parseTuiFlags([]);
    expect(without.allowSideEffects).toBe(false);
    const withFlag = parseTuiFlags(["--until-pass", "bun", "--allow-side-effects"]);
