...
describe("parseStartFlags — --until-pass / --max-iter (#1624)", () => {
  test("no flag → untilPass is empty, maxIter defaults to 10", () => {
    const flags = parseStartFlags(["--prompt", "hi"], GLOBAL);
  ...
  test("single --until-pass token is collected as 1-element argv", () => {
    const flags = parseStartFlags(
      ["--prompt", "fix it", "--until-pass", "bun", "--allow-side-effects"],
      GLOBAL,
    );
  ...
  test("--max-iter parses to integer", () => {
    const flags = parseStartFlags(
      ["--prompt", "hi", "--until-pass", "bun", "--max-iter", "5", "--allow-side-effects"],
      GLOBAL,
    );
  ...
  test("--max-iter rejects non-integer", () => {
    expect(() => parseStartFlags(["--prompt", "hi", "--max-iter", "not-a-number"], GLOBAL)).toThrow(
      ParseError,
    );
  });
  ...
  test("regression: --max-iter rejects strings with trailing junk (round 40)", () => {
    ...
    expect(() => parseStartFlags(["--prompt", "hi", "--max-iter", "10abc"], GLOBAL)).toThrow(
      ParseError,
    );
  });
  ...
  test("--max-iter rejects zero / negative", () => {
    expect(() => parseStartFlags(["--prompt", "hi", "--max-iter", "0"], GLOBAL)).toThrow(
      ParseError,
    );
    expect(() => parseStartFlags(["--prompt", "hi", "--max-iter", "-1"], GLOBAL)).toThrow(
      ParseError,
    );
  });

  test("empty --until-pass token is rejected", () => {
    expect(() => parseStartFlags(["--prompt", "hi", "--until-pass", ""], GLOBAL)).toThrow(
      ParseError,
    );
  });
  ...
  test("--until-pass without --prompt is rejected (fail closed)", () => {
    ...
    expect(() => parseStartFlags(["--until-pass", "bun"], GLOBAL)).toThrow(ParseError);
  });
  ...
  test("--until-pass combined with --resume is rejected (silent-state-loss guard)", () => {
    ...
    expect(() =>
      parseStartFlags(["--prompt", "hi", "--until-pass", "bun", "--resume", "ses_abc"], GLOBAL),
    ).toThrow(ParseError);
  });

  test("--verifier-timeout parses to milliseconds with default 120_000", () => {
    const defaults = parseStartFlags(
      ["--prompt", "hi", "--until-pass", "bun", "--allow-side-effects"],
      GLOBAL,
    );
  ...
  test("--until-pass without --allow-side-effects is rejected (round-11 trust boundary guard)", () => {
    ...
    // proceeding.
    expect(() => parseStartFlags(["--prompt", "hi", "--until-pass", "bun"], GLOBAL)).toThrow(
      ParseError,
    );
  });
  ...
  test("--until-pass combined with --log-format json is rejected", () => {
    ...
    expect(() =>
      parseStartFlags(
        ["--prompt", "hi", "--until-pass", "bun", "--allow-side-effects", "--log-format", "json"],
        GLOBAL,
      ),
    ).toThrow(ParseError);
  });
  ...
  test("--until-pass with --log-format text is accepted", () => {
    ...
    const flags = parseStartFlags(
      ["--prompt", "hi", "--until-pass", "bun", "--allow-side-effects", "--log-format", "text"],
      GLOBAL,
    );
  ...
  test("--allow-side-effects is captured on the returned flags", () => {
    const without = parseStartFlags(["--prompt", "hi"], GLOBAL);
