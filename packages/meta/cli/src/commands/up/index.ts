...
export async function runUp(flags: UpFlags): Promise<void> {
  ...
  // 8c. Seed callable forged tool brick into forge store (demo preset only).
  // This creates a real forged tool that the LLM can call, generating live
  // forge fitness events visible in the TUI Forge view.
  if (forgeBootstrap !== undefined && demoPack !== undefined) {
    try {
      const { brickId, DEFAULT_SANDBOXED_POLICY } = await import("@koi/core");
      const now = Date.now();
      const exists = await forgeBootstrap.store.exists(
        brickId("sha256:demo-forged-calculator-001"),
      );
      if (!exists.ok || !exists.value) {
        await forgeBootstrap.store.save({
          id: brickId("sha256:demo-forged-calculator-001"),
          kind: "tool",
          name: "forged_calculator",
          description:
            "A forged calculator tool created by the forge system. Adds, subtracts, multiplies, or divides two numbers. ALWAYS use this for arithmetic.",
          scope: "agent",
          origin: "forged",
          policy: DEFAULT_SANDBOXED_POLICY,
          lifecycle: "active",
          provenance: {
            source: { origin: "forged", forgedBy: manifest.name, sessionId: currentSessionId },
            buildDefinition: { buildType: "koi.forge/tool/v1", externalParameters: {} },
            builder: { id: "koi.forge/pipeline/v1", version: "0.1.0" },
            metadata: {
              invocationId: `demo-seed-${String(now)}`,
              startedAt: now - 1000,
              finishedAt: now,
            ...
            contentMarkers: [],
            contentHash: `demo-calc-${String(now)}`,
          },
          version: "0.1.0",
          tags: ["demo", "forged", "calculator"],
          usageCount: 0,
          implementation: [
            ...
            "else if (op === 'divide') result = b !== 0 ? a / b : 'error: division by zero';",
            "else result = a + b;",
        ...
        output.debug("Seeded forged_calculator into forge store");
      }
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      process.stderr.write(`warn: failed to seed forged_calculator: ${msg}\n`);
    }
  }
