import type { CapabilityFragment, KoiMiddleware, ModelChunk, ModelHandler, ModelRequest, ModelResponse, ModelStreamHandler, TurnContext } from "@koi/core";
...
function renderBlock(tag: "mounted_connectors" | "runtime_mounted_connectors", entries: readonly MountDescription[]): string | undefined {
  ...
  const items = entries
    .map((entry) => {
      ...
      if (entry.description !== undefined && entry.description.length > 0) {
        attrs.push(`description="${escapeXmlAttr(entry.description)}"`);
      }
...
function joinPromptSections(sections: readonly (string | undefined)[]): string | undefined {
  const filtered = sections.filter((section): section is string => section !== undefined && section.length > 0);
