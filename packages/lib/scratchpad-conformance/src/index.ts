...
export function describeScratchpadConformance(label: string, factory: ScratchpadFactory): void {
  describe(`ScratchpadComponent conformance: ${label}`, () => {
    test("write/read round-trip preserves content and assigns generation 1", async () => {
    ...
    test("read of missing path returns NOT_FOUND, not throws", async () => {
      const pad = await factory();
    ...
    test("unconditional write overwrites and increments generation", async () => {
      const pad = await factory();
    ...
    test("CAS create-only (expectedGeneration=0) fails CONFLICT when path exists", async () => {
      const pad = await factory();
    ...
    test("CAS update with stale generation fails CONFLICT", async () => {
      const pad = await factory();
    ...
    test("delete removes the entry and is observable via list/read", async () => {
      const pad = await factory();
    ...
    test("list returns summaries (no content) for all written entries", async () => {
      const pad = await factory();
      ...
      // Summaries omit `content` — must not be present
      for (const summary of list) {
        expect(Object.hasOwn(summary, "content")).toBe(false);
      }
    });

    test("onChange eventually delivers a 'written' event reflecting the latest generation", async () => {
    ...
    test("onChange does NOT replay historical state to a new subscriber", async () => {
      const pad = await factory();
      ...
      const unsub = pad.onChange((e) => events.push(e));
      try {
        // Wait a beat — if the adapter replays, we'd see the seeded entry here.
        await new Promise((r) => setTimeout(r, 200));
        expect(events.filter((e) => e.path === path).length).toBe(0);

        // New write AFTER subscription must be observed.
    ...
    test("a throwing subscriber must not stop sibling subscribers from receiving events", async () => {
      const pad = await factory();
