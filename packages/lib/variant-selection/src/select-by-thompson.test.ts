...
function makeSeededRandom(seed: number): () => number {
  // let needed: seeded LCG mutates state on each call.
  let s = seed;
  return () => {
...
function makeCtx(random: () => number): { clock: () => number; random: () => number } {
...
describe("updateThompson", () => {
  ...
  test("accumulates correctly over multiple updates", () => {
    // let needed: re-bound across sequential immutable updates.
...
describe("selectByThompson", () => {
  ...
  test("variant with many successes is selected more often", () => {
    ...
    // let needed: counter accumulates across runs.
    let goodCount = 0;
    const runs = 1000;
