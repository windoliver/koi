...
describe("validateToolAuditConfig", () => {
  ...
  test("rejects null", () => {
    expect(validateToolAuditConfig(null).ok).toBe(false);
  });

  test("rejects undefined", () => {
    expect(validateToolAuditConfig(undefined).ok).toBe(false);
  });

  test("rejects non-object", () => {
    expect(validateToolAuditConfig("string").ok).toBe(false);
  });
  ...
  describe("numeric threshold validation", () => {
    ...
    for (const field of positiveFields) {
      ...
      test(`rejects ${field} = -1`, () => {
        expect(validateToolAuditConfig({ [field]: -1 }).ok).toBe(false);
      });

      test(`rejects ${field} = NaN`, () => {
        expect(validateToolAuditConfig({ [field]: Number.NaN }).ok).toBe(false);
      });

      test(`rejects ${field} = Infinity`, () => {
        expect(validateToolAuditConfig({ [field]: Number.POSITIVE_INFINITY }).ok).toBe(false);
      });

      test(`rejects ${field} = "string"`, () => {
        expect(validateToolAuditConfig({ [field]: "ten" }).ok).toBe(false);
      });

      test(`accepts ${field} = 1`, () => {
        expect(validateToolAuditConfig({ [field]: 1 }).ok).toBe(true);
      });
    }
    ...
    for (const field of ratioFields) {
      ...
      test(`rejects ${field} = 1.1`, () => {
        expect(validateToolAuditConfig({ [field]: 1.1 }).ok).toBe(false);
      });

      test(`rejects ${field} = "string"`, () => {
        expect(validateToolAuditConfig({ [field]: "half" }).ok).toBe(false);
      });

      test(`accepts ${field} = 0`, () => {
        expect(validateToolAuditConfig({ [field]: 0 }).ok).toBe(true);
      });
      ...
      test(`accepts ${field} = 0.5`, () => {
        expect(validateToolAuditConfig({ [field]: 0.5 }).ok).toBe(true);
      });
    }
  });
