 ...
 *
 * Used by Meta Llama 3.1+ instruction-tuned models. The tool name comes from
 * the attribute (`function=NAME`) and arguments from the body JSON.
 */
...
export const llama31Pattern: ToolCallPattern = {
  ...
  detect(text: string): RecoveryResult | undefined {
    ...
    for (const match of matches) {
      ...
      // Malformed body → fall through to next pattern.
