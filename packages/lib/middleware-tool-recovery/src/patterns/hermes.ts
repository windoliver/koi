/**
 * Hermes / ChatML tool call pattern.
 *
 * Matches: `<tool_call>{"name": "...", "arguments": {...}}</tool_call>`
 *
 * Used by NousResearch Hermes models and ChatML-based fine-tunes.
 */
...
export const hermesPattern: ToolCallPattern = {
  ...
  detect(text: string): RecoveryResult | undefined {
    ...
    for (const match of matches) {
      ...
      // Malformed Hermes tag → reject the whole pattern (the entire response
      // is suspect). The orchestrator falls through to the next pattern.
