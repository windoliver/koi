...
import type { RetrySignalWriter } from "@koi/core/retry-signal";
...
export interface SemanticRetryConfig {
  ...
  /**
   * Optional retry signal writer for cross-middleware coordination.
   * When provided, sets retry signals that event-trace can read to
   * annotate trajectory steps with retry metadata.
   */
  readonly signalWriter?: RetrySignalWriter | undefined;
}
