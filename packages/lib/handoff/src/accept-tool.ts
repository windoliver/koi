/**
 * accept_handoff tool factory — accepts and unpacks a HandoffEnvelope,
 * returning full context to the receiving agent.
 */
...
async function accept(config: CreateAcceptToolConfig, id: HandoffId): Promise<HandoffAcceptResult> {
  ...
  if (envelope.to !== config.agentId) {
    return {
      ok: false,
      error: { code: "TARGET_MISMATCH", expected: envelope.to, actual: config.agentId },
    };
  }
...
function makeError(
  ...
  return {
    output: null,
    metadata: { error: { code, message, retryable: false } },
  };
}
