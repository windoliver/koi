...
export function parseUploadFiles(
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    // Derive MIME from magic bytes only — never trust the filename extension
    // for uploads, as the extension can be spoofed to bypass accept filters or
    // downstream MIME validators. Strong magic-byte matches are authoritative;
    ...
    const resolvedMime = (() => {
      ...
      if (detected !== null && detected.confidence === "strong") return detected.mimeType;
 ...
 * Returns the url string on success, a VALIDATION error on failure.
 */
export function validateUrlScheme(url: string): ParseResult<string> {
  let parsed: URL;
  try {
    parsed = new URL(url);
  } catch {
    return { ok: false, err: { error: `Invalid URL: ${url}`, code: "VALIDATION" } };
  }
  if (!(ALLOWED_URL_SCHEMES as readonly string[]).includes(parsed.protocol)) {
    return {
      ok: false,
      err: {
        error: `URL scheme "${parsed.protocol}" is not allowed. Only http:// and https:// are supported.`,
        code: "VALIDATION",
      },
    };
  }
