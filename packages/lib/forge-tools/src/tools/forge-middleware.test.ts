...
import { createForgeMiddlewareTool, type ForgeMiddlewareOk } from "./forge-middleware.js";
...
function asResult(value: unknown): Result<ForgeMiddlewareOk, KoiError> {
  if (
    typeof value !== "object" ||
    value === null ||
    !("ok" in value) ||
  ...
  return value as Result<ForgeMiddlewareOk, KoiError>;
}

const validArgs: JsonObject = {
  name: "logging-mw",
  description: "Logs every model + tool call.",
  version: "0.0.1",
  scope: "agent",
  implementation: "export default async function (next) { /* ... */ }",
};
...
describe("forge_middleware", () => {
  test("synthesizes ImplementationArtifact (kind: middleware) with lifecycle: draft", async () => {
    const tool = createForgeMiddlewareTool({ store });
    ...
    expect(loaded.value.kind).toBe("middleware");
