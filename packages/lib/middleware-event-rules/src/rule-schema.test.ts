...
describe("validateEventRulesConfig — valid configs", () => {
  ...
  test("accepts config with condition (count + window)", () => {
    ...
    if (result.ok) {
      ...
      expect(rule.condition).toEqual({ count: 3, window: "1m", windowMs: 60_000 });
    }
  });
  ...
  test('oneOf array preserves primitive types — `1` does not match `"1"`', () => {
    const result = validateEventRulesConfig({
      rules: [
        {
          name: "numeric-oneof",
          on: "turn_complete",
          match: { turnIndex: [1, 2, 3] },
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      // biome-ignore lint/style/noNonNullAssertion: validated single rule + predicate
      const pred = result.value.rules[0]!.predicates[0]!;
      expect(pred.test(1)).toBe(true);
      expect(pred.test("1")).toBe(false);
      expect(pred.test(true)).toBe(false); // 1 == true in loose equality, must not match
    }
  });

  test('oneOf array preserves boolean type — `true` does not match `"true"`', () => {
    const result = validateEventRulesConfig({
      rules: [
        {
          name: "bool-oneof",
    ...
    if (result.ok) {
      // biome-ignore lint/style/noNonNullAssertion: validated single rule + predicate
      const pred = result.value.rules[0]!.predicates[0]!;
      expect(pred.test(true)).toBe(true);
...
describe("validateEventRulesConfig — rejections", () => {
  ...
  test("rejects tool_call typo of core field (toolID instead of toolId)", () => {
    const result = validateEventRulesConfig({
      rules: [
        {
          name: "typo",
  ...
  test("rejects tool_call typo of core field (sessionID instead of sessionId)", () => {
    const result = validateEventRulesConfig({
      rules: [
        {
          name: "typo",
  ...
  test("rejects regex patterns vulnerable to catastrophic backtracking", () => {
    for (const bad of ["(a+)+", "(a*)*b", "(.+)+x", "(a|a)*", "(foo|fo)+"]) {
      const result = validateEventRulesConfig({
        rules: [
          {
            name: "redos",
            on: "tool_call",
            match: { command: { regex: bad } },
  ...
  test("rejects regex patterns longer than the safe-length cap", () => {
      ...
      rules: [
        {
          name: "long",
          on: "tool_call",
          match: { command: { regex: longPattern } },
          actions: [{ type: "log", level: "warn", message: "x" }],
        },
      ],
    });
  ...
  test("rejects tool_call typo of core field within edit-distance one (sessoinId, trunIndex)", () => {
    for (const bad of ["sessoinId", "trunIndex", "toolld", "agentld", "blockd"]) {
      const result = validateEventRulesConfig({
        rules: [
          {
            name: `typo-${bad}`,
  ...
  test("accepts tool_call match with custom tool-input field (not a core typo)", () => {
    const result = validateEventRulesConfig({
      rules: [
        {
          name: "input-field",
          on: "tool_call",
          match: { command: "rm -rf /" }, // tool-specific input
          actions: [{ type: "log", level: "warn", message: "x" }],
        },
      ],
    });
  ...
  test("rejects unknown match field on session_start (typo guard)", () => {
    const result = validateEventRulesConfig({
      rules: [
        {
          name: "typo",
          on: "session_start",
          match: { toolId: "shell_exec" }, // toolId is not a session_start field
          actions: [{ type: "log", level: "info", message: "x" }],
        },
      ],
    });
  ...
  test("rejects unknown match field on turn_complete", () => {
    const result = validateEventRulesConfig({
      rules: [
        {
          name: "typo",
          on: "turn_complete",
          match: { ok: false }, // ok is not a turn_complete field
          actions: [{ type: "log", level: "info", message: "x" }],
        },
      ],
    });
  ...
  test("rejects condition.count above counter capacity (1000)", () => {
    const result = validateEventRulesConfig({
      rules: [
        {
          name: "huge-count",
          on: "tool_call",
          condition: { count: 1001, window: "1m" },
          actions: [{ type: "log", level: "info", message: "x" }],
        },
      ],
    });
  ...
  test("accepts condition.count at counter capacity (1000)", () => {
    const result = validateEventRulesConfig({
      rules: [
        {
          name: "max-count",
          on: "tool_call",
          condition: { count: 1000, window: "1m" },
          actions: [{ type: "log", level: "info", message: "x" }],
        },
      ],
    });
