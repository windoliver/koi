...
async function testNameAndSandboxed(factory: WorkspaceFactory): Promise<void> {
...
async function testCreateShape(factory: WorkspaceFactory): Promise<void> {
  const { backend, cleanup } = await factory();
...
async function testIsHealthy(factory: WorkspaceFactory): Promise<void> {
  const { backend, cleanup } = await factory();
...
async function testDoubleDispose(factory: WorkspaceFactory): Promise<void> {
...
async function testExistsAfterDispose(factory: WorkspaceFactory): Promise<void> {
  const { backend, cleanup } = await factory();
  try {
...
async function testFindByAgentId(factory: WorkspaceFactory): Promise<void> {
  const { backend, cleanup } = await factory();
  try {
...
async function testAttestationTriple(factory: WorkspaceFactory): Promise<void> {
  const { backend, cleanup } = await factory();
  try {
    if (
...
export function describeWorkspaceConformance(label: string, factory: WorkspaceFactory): void {
  describe(`WorkspaceBackend conformance: ${label}`, () => {
    test("declares a non-empty `name` and a boolean `isSandboxed`", () =>
      testNameAndSandboxed(factory));
    test("create returns a workspace with id/path/createdAt/metadata", () =>
      testCreateShape(factory));
    test("isHealthy returns true for a freshly-created workspace", () => testIsHealthy(factory));
    test("second dispose does not throw and returns a typed Result", () =>
      testDoubleDispose(factory));
    test("exists (when supported) flips to false after dispose", () =>
      testExistsAfterDispose(factory));
    test("findByAgentId (when supported) lists survivors for the same agent", () =>
      testFindByAgentId(factory));
    test("attestation hooks (when supported) form a consistent triple", () =>
      testAttestationTriple(factory));
  });
}
