...
describe("createWebExecutor.fetch caching", () => {
  ...
  // NOTE: the previous "does not alias GET requests that differ by body"
  // regression is now enforced upstream: `@koi/url-safety`'s createSafeFetcher
  // rejects GET/HEAD with a body as TypeError (matching native fetch), so the
  // aliasing path is unreachable through this executor. The defensive
  // `!hasRequestBody` gate on keyCacheable stays as belt-and-braces.
