/**
 * Unit tests for createSandboxCommand() — shared sandbox command construction.
 *
 * Tests cover:
...
describe("createSandboxCommand", () => {
  ...
  test("includes command arguments in output", () => {
    const result = createSandboxCommand(profile, "/bin/echo", ["arg1", "arg2"]);
  ...
  test("empty args array produces valid command", () => {
    const result = createSandboxCommand(profile, "/bin/echo", []);
  ...
  test("preserves command path in output args", () => {
    const result = createSandboxCommand(profile, "/usr/local/bin/myapp", ["--flag", "value"]);
  ...
  test("args with spaces are preserved", () => {
    const result = createSandboxCommand(profile, "/bin/echo", ["hello world", "foo bar"]);
  ...
  test("different profile tiers produce valid commands", () => {
    ...
    const result = createSandboxCommand(minimalProfile, "/bin/echo", ["test"]);
