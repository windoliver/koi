 ...
 * All mutable state is closure-scoped with `// let justified:` comments.
 */
...
export function createPressureTrendTracker(
  ...
  function compute(threshold: number): ContextPressureTrend {
    ...
    // Endpoint-to-endpoint average: avoids O(n) least-squares for a hot path.
    // Trade-off: intermediate spikes are invisible; acceptable for trend estimation.
