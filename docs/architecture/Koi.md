...
<<<<<<< HEAD
**L0-utility packages** (14 total — canonical list lives in `scripts/layers.ts` → `L0U_PACKAGES`):
`@koi/channel-base`, `@koi/config`, `@koi/context-manager`, `@koi/edit-match`, `@koi/errors`, `@koi/event-delivery`, `@koi/execution-context`, `@koi/file-resolution`, `@koi/git-utils`, `@koi/hash`, `@koi/session-repair`, `@koi/shutdown`, `@koi/token-estimator`, `@koi/validation`.
=======
