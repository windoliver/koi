...
> **Workspace filter gotcha (#1788)**: `--filter=<pkg>` is a Turborepo workspace selector and must be passed via `bun run test`, which delegates to `turbo run test` (the root `test` script). The bare Bun runner has no workspace filter of its own; passing `--filter` directly silently walks every workspace. From a single package directory, `cd packages/meta/runtime` and run the suite without any filter flag. The `.github/workflows/check-bun-test-filter.yml` CI workflow enforces this — see `scripts/check-bun-test-filter.ts` and its unit tests for the exact patterns it rejects.
