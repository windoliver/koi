...
- **`tar`** — `-c` create: **`partial`, `reason: "recursive-subtree-root"`**, `-f FILE` → `writes`; positional file list → `reads` (with `-C DIR` rebase if present). Always partial because any operand may be a directory whose descendants tar reads recursively, and argv alone cannot enumerate the descendant set; reported reads act as subtree roots. `-t` list: `complete`, `-f FILE` → `reads`; no writes. `-x` extract: `partial`, `reason: "tar-extract-targets-in-archive"`, `-f FILE` → `reads` (the archive itself); `writes: []` (extraction targets inside the archive may escape any `-C DIR` via absolute paths or `..` traversal).
