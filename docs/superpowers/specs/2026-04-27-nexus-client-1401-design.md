...
7h_unc10b. `EXPLICIT opt-out + assert-* + NO nexusTransport supplied: BOOTS without throw (no transport means no Nexus block exists to bypass; stale boot mode is dead config but cannot leak security guarantees). No warning either — telemetry is the default behavior already`
...
| `nexusTransport` + local-bridge (no flags) | THROWS at boot with actionable message naming both flags AND the explicit fs-only opt-out ("set both nexusPermissionsEnabled=false AND nexusAuditEnabled=false"). | If fs-only local-bridge: set BOTH consumer flags explicitly to `false` (entrypoints continue passing nexusTransport — Loop 5 R10). If wiring Nexus consumers: set both flags explicitly to `true`. |
