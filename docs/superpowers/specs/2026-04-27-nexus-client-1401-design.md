...
export interface NexusHealthOptions {
  ...
  /**
   * Per-call deadline applied to the version probe AND each read probe in
   * `readPaths`. Defaults to `HEALTH_DEADLINE_MS = 5_000`. Earlier drafts
   * hard-coded the 5s gate inside `health()`, which made the assert-* boot
   * deadlines unreachable on slow-but-healthy Nexus deployments — the probe
   * timed out before `nexusBootSyncDeadlineMs` was consulted (Loop 5 R5
   * finding). Exposed as a configurable knob so the runtime can thread
   * `config.nexusProbeDeadlineMs` here, giving operators a single observable
   * startup-budget model: total ≈ probeDeadlineMs + bootSyncDeadlineMs.
   */
...
7. Per-call `deadlineMs` overrides each transport's default. The deadline is **CONFIGURABLE** via `NexusHealthOptions.probeDeadlineMs` (default `HEALTH_DEADLINE_MS = 5_000`). Runtime-factory threads `config.nexusProbeDeadlineMs` (env: `KOI_NEXUS_PROBE_DEADLINE_MS`, CLI: `--nexus-probe-deadline-ms`) into each `health()` call. The earlier hard-coded 5s gate prevented operators with legitimate cold-start or cross-region latency from passing the assert-* gate even after raising `nexusBootSyncDeadlineMs` — the probe ran first and timed out before the configurable knob was consulted (Loop 5 R5 finding). With `probeDeadlineMs` exposed, operators have a single observable startup-budget knob: `total_boot_budget ≈ probeDeadlineMs + bootSyncDeadlineMs` for `assert-remote-policy-loaded-at-boot`, or just `probeDeadlineMs` for `assert-transport-reachable-at-boot`.
...
**Opt-in (`nexusAuditPoisonOnError: true`):** Post-failure containment — first failure latches `nexusPoison`; subsequent `log()` calls on the wrapped sink reject with `code: "AUDIT_SINK_POISONED"`; the audit-middleware admission gate denies new work at the next boundary so no new tool/model call executes under a known-bad audit path. **This is intentionally weaker than NDJSON/SQLite required-sink semantics** (Loop 5 R8 — earlier wording falsely claimed parity): NDJSON/SQLite are local-disk and synchronous, so they can synchronously flush+rethrow per-operation; Nexus is remote and asynchronous, so per-operation synchronous flush is architecturally impossible without paying a remote round-trip per loop iteration. Already-admitted in-flight work runs to natural completion (no in-flight rollback — the engine does not expose per-operation cancellation). Operators requiring true per-operation durable audit + fail-stop must use a required local sink. Compatible with any `nexusBootMode`. See "Persistent design decision: nexusAuditPoisonOnError is post-failure containment" for the locked contract.
...
**Conditional rejection (config validation, refined Loop 5 R4):** the runtime THROWS at boot if `nexusTransport.kind === "local-bridge" && nexusAuditPoisonOnError === true` **AND the Nexus audit sink would actually be wired** (i.e., `nexusAuditMode` is unset or any future value that wires the sink). When `nexusAuditMode === "disabled"` or `"local-only"`, the Nexus sink is skipped — there is nothing to poison and the flag is inert; the runtime emits a one-time WARN naming the dead flag and continues. This narrows the earlier "throw unconditionally on this combination" rule because that rule turned a stale poison flag into an availability outage during rollback (Loop 5 R4 finding). Operators wanting fail-stop Nexus audit MUST use HTTP transport (the local-bridge wired-sink case); operators rolling back to `disabled`/`local-only` keep working with a clear warning. The error message names the conflict and the resolution paths.
...
interface KoiRuntimeFactoryConfig {
   ...
   *   explicit opt-out by setting both consumer flags to `false`).
   * **Phase 2 (next release):** unset throws on all transports.
   */
   ...
   * actionable message. Default: 10_000ms (probeDeadlineMs default 5s +
   * one read RTT round-trip budget for the backend's first sync — both
   * are independently tunable via nexusProbeDeadlineMs).
  ...
  /**
   * Per-call deadline applied to every health() probe (version + each
   * readPath). Default: HEALTH_DEADLINE_MS (5_000) inside the transport
   * when undefined here. Operators with legitimate cold-start or cross-
   * region Nexus latency can raise this to give the probe a larger
   * budget independently of nexusBootSyncDeadlineMs (Loop 5 R5).
...
export async function createKoiRuntime(config: KoiRuntimeFactoryConfig) {
  ...
  // Step -1: RUNTIME-LEVEL silent-bypass guard (Loop 5 R9, refined R10).
  // The CLI parser (parseNexusConfigFromEnvAndFlags) enforces this same
  // check, but KoiRuntimeFactoryConfig is a public runtime API —
  // programmatic callers that bypass the CLI must ALSO be protected.
  // Validation is duplicated by design: each public boundary (CLI parse +
  // runtime entry) enforces the same contract independently so neither
  // can be bypassed.
  //
  // Phase 1 reconciliation (Loop 5 R10): an HTTP transport supplied with
  // unset consumer flags will be inferred TRUE+warn in Step 0 (legacy
  // implicit-consumer wiring preserved). Treat that as implicit consumer
  // intent here so control-plane flags don't hard-fail legacy HTTP
  // deployments that ALSO add a boot-mode/probe knob during the same
  // upgrade. Without this carve-out, the guard would convert "legacy HTTP
  // + add KOI_NEXUS_BOOT_MODE=assert-*" into a startup outage instead of
  ...
  const httpTransportImpliesConsumer =
    ...
    && config.nexusPermissionsEnabled === undefined
    && config.nexusAuditEnabled === undefined;
  // Rollback escape hatch (Loop 4 R7 + Loop 6 R4): EXPLICIT opt-out
  // (both consumer flags false) AND no transport supplied means the
  // operator has fully disabled Nexus — there is no transport to
  // probe and no consumer to enforce against. Stale assert-* / probe-
  // deadline / policy-path env vars from a previous configuration are
  // dead config and must NOT block rollback by tripping the silent-
  // bypass guard. Without this carve-out, operators rolling back from
  // a Nexus-enabled deployment would have to scrub every Nexus env
  // var before the runtime would boot — defeating the locked rollback
  // contract that says explicit opt-out is the supported off switch.
  const fullyOptedOutWithNoTransport =
    config.nexusTransport === undefined
  ...
  // not supplied a transport — that combination is the documented
  // rollback path and stale env vars are inert by definition.
  ...
  //     inference path. Operators wanting fs-only local-bridge MUST
  //     EXPLICITLY set both nexusPermissionsEnabled=false AND
  //     nexusAuditEnabled=false (the documented explicit opt-out). The
  //     entrypoints (tui-command.ts, commands/start.ts) DO pass
  //     `nexusTransport` whenever they construct one — Phase 1 legacy
  //     preservation (Loop 5 R10) — so the throw fires here at the
  //     right boundary. Throwing forces the explicit decision —
  //     silent disablement of a security control is not an option.
  //     Programmatic callers that miss the migration get a hard,
  //     actionable error instead of a quiet authorization downgrade.
  ...
  // will hit the throw above the moment they raise the boot mode without also
  ...
  readPaths: probePolicyPaths,
  // Operator-controlled probe budget — defaults to HEALTH_DEADLINE_MS
  // (5000) inside the transport when undefined. Exposing this knob is
  // the runtime-side half of the Loop 5 R5 fix; without threading it
  // here, raising the env/CLI value would have no effect.
  ...
  if (txKind !== undefined && permsEnabled === true) {
    ...
    if ((config.nexusBootMode ?? "telemetry") === "assert-remote-policy-loaded-at-boot") {
      ...
      // the configurable probeDeadlineMs (default 5s) we used for the probe.
      // Without an
...
- Operators wanting boot-time visibility into transport reachability or remote-policy presence opt into the assert-* modes explicitly. (NB: per the framing two paragraphs above, these modes are STARTUP DIAGNOSTIC GATES, not security/compliance controls. Any reader reaching this section looking for compliance enforcement should refer to the "Out of scope" entries on centralized-policy enforcement and policy freshness — neither boot mode in this PR provides that contract, and recommending them for compliance would be a false-safety signal.)
...
| `packages/lib/nexus-client/src/health.test.ts` | New: all probes ok → `status:"ok"` + probed lists each path; either read returns 404 → `status:"missing-paths"` + notFound lists the 404'd paths (NEVER `status:"ok"` — 404 on a required probe path is fail-closed by design, see Loop 4 R1 status enum); empty readPaths → `status:"version-only"`; version 5xx → fail; any read 5xx → fail; read 401 → fail; malformed read body → VALIDATION error; per-call deadline honored; network error; malformed version response; nonInteractive flag set on all calls; **custom readPaths override default** (probes provided paths only); signal abort during call → code:"ABORTED" | +220 |
...
| `packages/meta/cli/src/tui-command.ts` | Drop the `as unknown as NexusTransport` cast at line 1704 — fs-nexus local-bridge transport now structurally satisfies base `NexusTransport` directly (after fs-nexus adds `kind: "local-bridge"` discriminator). **Phase 1 entrypoint behavior (Loop 5 R10 — supersedes earlier "decoupling"):** Pass `nexusTransport` to `createKoiRuntime` whenever the entrypoint has constructed one for the fs-nexus filesystem path (i.e., legacy behavior preserved). The runtime's Step 0 inference (HTTP unset → infer TRUE + warn) then runs as designed; legacy TUI HTTP deployments do NOT silently lose centralized-permissions / Nexus-audit on upgrade. For LOCAL-BRIDGE transports, the runtime throws unless both consumer flags are explicit (no inference path on local-bridge — see Step 0); operators wanting a fs-only local-bridge session must EXPLICITLY set both consumer flags to `false` (the documented opt-out) so the entrypoint's intent is unambiguous. The earlier "decoupling" rule (don't pass transport when no consumer wanted) was a Loop 4 attempt to simplify the upgrade tradeoff; it accidentally turned the implicit-HTTP-consumer path into a silent opt-out (Loop 5 R10 finding) and is therefore reversed. Phase 2 will harmonize: HTTP unset throws (no inference); operators on either transport must set flags explicitly; entrypoints continue to pass transport unchanged so the throw fires at the right boundary. | +25 |
| `packages/meta/cli/src/nexus-config.ts` | **NEW shared module** — single source of truth for parsing `KOI_NEXUS_*` env vars and `--nexus-*` CLI flags into the `KoiRuntimeFactoryConfig` Nexus subset. Exported `parseNexusConfigFromEnvAndFlags(env, flags) → Pick<KoiRuntimeFactoryConfig, "nexusPermissionsEnabled" \| "nexusAuditEnabled" \| "nexusAuditMode" \| "nexusAuditPoisonOnError" \| "nexusBootMode" \| "nexusPolicyPath" \| "nexusSyncIntervalMs" \| "nexusBootSyncDeadlineMs" \| "nexusProbeDeadlineMs" \| "nexusAllowEmptyPolicyStore">`. Used by BOTH `tui-command.ts` AND `commands/start.ts` so the new boot gate, consumer wiring, and assert-* preflight are uniform across interactive and headless entrypoints. **Parser scope (Loop 6 R6 — finalized):** parses tri-state booleans, validates value ranges + mutual exclusion, and passes all fields through to the runtime. Does NOT enforce silent-bypass — the runtime's Step -1 guard is the single source of truth (it has transport-presence context the parser cannot see, required for the locked rollback carve-out + Phase 1 HTTP-implicit-consumer carve-out). Error names every offending flag plus two resolution paths (enable a consumer / clear the control-plane flag). Without this module + guard, the headless path silently bypasses every new safety check. | +120 |
| `packages/meta/cli/src/commands/start.ts` | **Wire shared `parseNexusConfigFromEnvAndFlags(...)`** into the existing headless boot path that calls `createKoiRuntime(...)`. Today `koi start` only supplies a filesystem backend (`packages/meta/cli/src/commands/start.ts:493-584` and `:786-896`); after this PR it ALSO threads Nexus consumer flags + boot mode through to runtime config. **Phase 1 entrypoint behavior (Loop 5 R10 — symmetric with TUI):** Pass `nexusTransport` whenever the headless command has constructed one for the fs-nexus path (i.e., `manifest.filesystem.backend === "nexus"`). The runtime's Step 0 inference (HTTP unset → infer TRUE + warn) then fires; legacy headless HTTP Nexus deployments do NOT silently lose centralized-permissions / Nexus-audit on upgrade. They DO see the deprecation warning naming the explicit flags they should set for Phase 2. Headless deployments where `manifest.filesystem.backend !== "nexus"` continue to boot with no Nexus transport and no Nexus block — no change. Local-bridge headless boots throw at runtime Step 0 unless both consumer flags are explicit (no inference path on local-bridge); operators who want fs-only local-bridge MUST explicitly set both flags to `false`. | +75 |
| `packages/meta/cli/src/__tests__/start-nexus-wiring.test.ts` | New: headless `koi start` with `KOI_NEXUS_PERMISSIONS=true + KOI_NEXUS_BOOT_MODE=assert-remote-policy-loaded-at-boot` + healthy HTTP transport boots; same with unreachable transport throws (proves headless honors the gate); `KOI_NEXUS_AUDIT_POISON=true + KOI_NEXUS_AUDIT=true` headless propagates to runtime config; **legacy headless HTTP Nexus deployment (manifest.filesystem.backend==="nexus") with NO `KOI_NEXUS_*` flags: BOOTS, transport IS passed, runtime Step 0 infers permissions+audit=TRUE+warns (Loop 5 R10 — Phase 1 contract preserved end-to-end). Regression guard: pre-PR implicit centralized-permissions+audit wiring still works on upgrade with no operator action.** Headless without Nexus filesystem backend continues to boot with no Nexus transport (no change). **Silent-bypass guard with HTTP transport (Loop 5 R10 carve-out): headless HTTP + `KOI_NEXUS_BOOT_MODE=assert-*` + no consumer flags BOOTS — Phase 1 inference treats HTTP-transport-supplied as implicit consumer intent, control-plane flags reach the runtime. Same input WITHOUT a Nexus transport throws (no consumer to wire).** Symmetric test through tui-command.ts. | +180 |
| `packages/meta/cli/src/__tests__/nexus-config.test.ts` | New: tri-state parsing for every flag; mutual-exclusion enforcement for `--nexus-*` / `--no-nexus-*`; `KOI_NEXUS_BOOT_MODE` rejects unknown values with actionable message; legacy `require` audit-mode rejected; numeric fields parse + bound-check; **parser-scope tests (Loop 6 R6 — finalized): every field parses tri-state correctly and reaches createKoiRuntime config; legacy headless deployment with NO Nexus flags continues to boot unchanged; combinations like control-plane flags + opt-out flags are PASSED THROUGH unchanged (no parser-level throw); the runtime's Step -1 guard is responsible for cross-validation that depends on transport presence. nexusAllowEmptyPolicyStore parses tri-state (true/1/false/0/unset) from KOI_NEXUS_ALLOW_EMPTY_POLICY_STORE env + --nexus-allow-empty-policy-store / --no-nexus-allow-empty-policy-store CLI; reaches createKoiRuntime config end-to-end (verified via integration test that sets the env var, parses it, passes it to the runtime, and confirms 404 boots-with-warning instead of throwing). Silent-bypass enforcement tests are in the runtime-factory test file (where transport-presence is controllable), not in the parser tests.** | +160 |
...
8. `health() probes execute within probeDeadlineMs (default HEALTH_DEADLINE_MS=5000) not transport default; runtime threads config.nexusProbeDeadlineMs into health() opts; raising the knob to 30s lets a slow-but-healthy probe complete past the default 5s without failing assert-* gates (Loop 5 R5 — single observable startup-budget knob)`
...
7g16j11. `local-bridge + auditEnabled=true + nexusAuditMode="disabled" + nexusBootMode=telemetry: probe SKIPPED (no consumer effectively wired on local-bridge with disabled-mode), boot continues. local-bridge + assert-* with the same input STILL THROWS at the boot-mode preflight gate (Loop 6 R6 — assert-* is HTTP-only regardless of effective-consumer state; effective-consumer skip applies only to probe work, not to mode validation)`
7g16j12. `local-bridge + auditEnabled=true + nexusAuditMode="local-only" + nexusBootMode=telemetry: probe SKIPPED, boot continues. Same input + assert-* THROWS at boot-mode preflight (same root rule as 7g16j11)`
...
7g16y. `Phase 1 local-bridge with no flags: THROWS with actionable message naming both flags AND the explicit fs-only opt-out ("set both nexusPermissionsEnabled=false AND nexusAuditEnabled=false")` (Loop 5 R10 — single behavior across spec/migration/tests; no inference; entrypoints DO pass nexusTransport — operators opt out via explicit flags)
...
**Default-preserving rule (Loop 5 R10 — supersedes Loop 4 R9 entrypoint decoupling):** `nexusTransport` is constructed and threaded into `createKoiRuntime` whenever an entrypoint already builds a Nexus transport for the filesystem path. This applies SYMMETRICALLY to BOTH `tui-command.ts` (interactive) and `commands/start.ts` (headless) — there is no host-driven asymmetry. Legacy HTTP TUI deployments with no `KOI_NEXUS_*` flags continue to receive implicit centralized-permissions + Nexus-audit wiring via the runtime's Step 0 HTTP-infer-TRUE+warn path (Phase 1) — exactly as pre-PR. The earlier "decoupling" rule (omit transport when no consumer wanted) accidentally turned the implicit-HTTP-consumer path into a silent opt-out and is reversed. Headless legacy deployments without a Nexus transport (the fs backend isn't Nexus) continue to boot with no Nexus block — no change in either direction. Local-bridge entrypoints follow the explicit-flag rule via the runtime's Step 0 throw (no inference), which forces operators to declare intent.

**Silent-bypass guard (Loop 5 R7, refined R10, finalized R6 R6):** the parser does NOT enforce silent-bypass — it passes all parsed fields through unchanged. The runtime's Step -1 guard is the SINGLE source of truth because it has full context (transport presence, fully-opted-out-with-no-transport carve-out for rollback, HTTP-implicit-consumer carve-out for Phase 1 legacy). An earlier draft duplicated the guard in the parser, but the parser cannot see `nexusTransport`, so it could not implement the `fullyOptedOutWithNoTransport` carve-out — which would block the locked rollback contract for fs-only deployments rolling back from a Nexus-enabled state with stale env vars (Loop 6 R6 finding). All silent-bypass enforcement happens in `createKoiRuntime` Step -1 where transport-presence is observable. The parser's job is purely tri-state parsing + value validation (mode names, numeric bounds, mutual-exclusion of flags); it does NOT cross-validate combinations that depend on transport. Symmetry between tui-command.ts and commands/start.ts is preserved because both pass the parsed config to the same `createKoiRuntime` boundary.
...
| `nexusTransport` + local-bridge (no flags) | THROWS at boot with actionable message naming both flags AND the explicit fs-only opt-out ("set both nexusPermissionsEnabled=false AND nexusAuditEnabled=false"). | If fs-only local-bridge: set BOTH consumer flags explicitly to `false` (entrypoints continue passing nexusTransport — Loop 5 R10). If wiring Nexus consumers: there is NO supported path on local-bridge — switch to HTTP transport (Nexus permissions on local-bridge is a locked rejection; Nexus audit on local-bridge is unsupported except via `nexusAuditMode=disabled/local-only`, both of which skip the Nexus sink). |
...
This row in the Phase 1 table has surfaced as a recurring review concern across multiple adversarial-review rounds (recorded across two review-loop runs — 6+ occurrences). Two options were repeatedly considered:
  ...
  (A) **Throw at boot** — chosen. Atomic in-tree migration: this PR updates every in-tree caller of `createKoiRuntime(...)` to set both consumer flags explicitly when wiring local-bridge. External programmatic callers using `createKoiRuntime` directly with a local-bridge transport must either (a) omit `nexusTransport` for fs-only programmatic uses, or (b) set both consumer flags explicitly (`false/false` for fs-only sessions, `true/true` for wired Nexus consumers). Failure mode is loud and actionable.
...
Decision (locked): option (A). External callers see a hard, actionable error instead of a silent authorization downgrade. In-tree callers are updated atomically to set both consumer flags explicitly; reviewers concerned about external callers should consult the migration table above for the explicit migration paths.
...
Operators who do NOT need Nexus consumers but still use local-bridge for fs reads use the explicit fs-only opt-out (`nexusPermissionsEnabled=false` + `nexusAuditEnabled=false`). The entrypoints continue to pass `nexusTransport` to `createKoiRuntime` (Loop 5 R10 — Phase 1 legacy preservation), and the runtime's Step 0 sees explicit-disabled flags and skips Nexus consumer wiring. This works today and does not break pre-PR fs-only deployments. The "outage" framing in adversarial review applies only to deployments that were relying on the pre-PR unsafe implicit wiring on local-bridge; those deployments need to migrate to HTTP, not preserve the bug.
...
### Persistent design decision: `nexusAuditPoisonOnError` is post-failure containment, not in-flight operation containment

Adversarial review recurringly observes that `nexusAuditPoisonOnError: true` does not stop work that is already in flight when the first audit failure lands — already-admitted tool/model calls run to natural completion, and the triggering record may already be lost in the background flush. This is a known and explicitly documented limitation of the mode (see the JSDoc on `nexusAuditPoisonOnError` for the full "what this flag does NOT do" list).

Why no further mitigation in this PR:

- **In-flight cancellation requires coordinated runtime shutdown** that the runtime does not currently expose (only `shutdownBackgroundTasks()`, fire-and-forget). Tracked under "Out of scope".
...
### Persistent design decision: boot modes are STARTUP gates, not runtime enforcement / freshness controls

Adversarial review recurringly asks for `assert-transport-reachable-at-boot` and `assert-remote-policy-loaded-at-boot` to either (a) block all request authorization until first sync completes (closing the pre-sync local-fallback window), or (b) reject queries when remote-policy sync age exceeds a bound (closing the stale-policy window after partition / 404 / parse failure). Both are legitimate concerns. Neither is in scope for this PR. They share a common upstream blocker: the existing permission composition (`runtime-factory.ts:1797-1806`) chains the Nexus backend to local TUI on `ask`/no-opinion results, so even the strictest boot mode in this PR cannot prevent local-rule fallback on queries the remote policy doesn't match — and adding a TTL/staleness gate without that composition change just produces a different surprise (deny on previously-allowed queries during partition).

Why no further mitigation in this PR:

- **Pre-sync fallback window** for `assert-transport-reachable-at-boot` is intentional. That mode's contract is exactly "transport carried a read at boot — nothing more". Operators who need stronger boot-time guarantees use `assert-remote-policy-loaded-at-boot`, which awaits `backend.ready` AND `isCentralizedPolicyActive()` BEFORE exposing the runtime to requests (the mode throws before `createKoiRuntime` returns). The pre-sync window does NOT exist in `assert-remote-policy-loaded-at-boot`.
- **Stale-policy after first sync** for `assert-remote-policy-loaded-at-boot` is documented in the mode description AND in the mode name itself (`-loaded-at-boot`, NOT `-enforced-continuously`). Adding a freshness gate is a runtime-mode change with its own design (TTL semantics, partition-recovery UX, fail-closed-on-stale interaction with permission composition). Tracked under "Out of scope: Policy freshness".
- **Local-fallback on unmatched queries** in BOTH modes is a permission-composition change tracked separately. The mode names deliberately do NOT include the word "enforce" so operators cannot mistake them for enforcement controls.

Reviewers who continue to ask this PR's boot modes to provide centralized enforcement, pre-sync request blocking, or runtime freshness guarantees should refer to the per-mode JSDoc + this section + the corresponding "Out of scope" entries. Decision is locked across review loops.
...
- **Server-side empty-store sentinel / positive bootstrap signal** — `nexusAllowEmptyPolicyStore=true` is a CLIENT-SIDE escape hatch where the operator asserts "I know my tenant is empty, please boot" because the client cannot distinguish empty-store 404s from typo / wrong-tenant / wrong-namespace 404s without a server-side signal (e.g., a sentinel `koi/permissions/.empty` marker that the server returns 200 for in greenfield deployments). Adding such a sentinel requires Nexus server changes + server-side conventions and is its own design. Until it lands, the client-side opt-in is the operator's explicit acknowledgement of the ambiguity — Codex review has correctly observed that this is a trust-boundary tradeoff, but the alternative (no opt-in at all) blocks every greenfield bootstrap deployment and the alternative (auto-detect) requires the missing server primitive. Decision is locked across review loops; reviewers asking for stricter client-side discrimination should refer here AND to the runtime-factory comment at the missing-paths handler.
...
- [ ] Issue #1401 closed by the FOLLOW-UP IMPLEMENTATION PR (NOT this design-only PR). This spec PR does not change runtime behavior; closing #1401 from here would mark Nexus health-check work complete while no runtime protections, config gates, or tests have landed.
