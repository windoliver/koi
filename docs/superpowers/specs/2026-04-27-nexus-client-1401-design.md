...
- Operators are advised NOT to combine `local-bridge` transport with `nexusAuditPoisonOnError: true` in environments where first audit write may precede first user auth. Runtime logs a warning at boot when this combination is configured.
- Deployments requiring fail-stop audit on Nexus SHOULD use HTTP transport (no auth-wedge surface).
...
interface KoiRuntimeFactoryConfig {
  ...
  readonly nexusAuditPoisonOnError?: boolean | undefined;
  /**
   * Required when `nexusTransport.kind === "local-bridge"`. Forces explicit
   * operator decision rather than silent degradation:
   ...
   * No default. Runtime throws if local-bridge transport is configured and
   * this field is unset. Ignored for HTTP transport (no wedge surface).
   */
