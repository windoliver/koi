...
| `packages/meta/cli/src/nexus-config.ts` | **NEW shared module** — single source of truth for parsing `KOI_NEXUS_*` env vars and `--nexus-*` CLI flags into the `KoiRuntimeFactoryConfig` Nexus subset. Exported `parseNexusConfigFromEnvAndFlags(env, flags) → Pick<KoiRuntimeFactoryConfig, "nexusPermissionsEnabled" \| "nexusAuditEnabled" \| "nexusAuditMode" \| "nexusAuditPoisonOnError" \| "nexusBootMode" \| "nexusPolicyPath" \| "nexusSyncIntervalMs" \| "nexusBootSyncDeadlineMs" \| "nexusProbeDeadlineMs">`. Used by BOTH `tui-command.ts` AND `commands/start.ts` so the new boot gate, consumer wiring, and assert-* preflight are uniform across interactive and headless entrypoints. **Silent-bypass guard (Loop 5 R7): THROWS when any control-plane flag (`nexusBootMode != telemetry`, `nexusAuditPoisonOnError === true`, `nexusProbeDeadlineMs`, `nexusBootSyncDeadlineMs`, non-default `nexusPolicyPath`) is set without an enabled consumer.** Error names every offending flag plus two resolution paths (enable a consumer / clear the control-plane flag). Without this module + guard, the headless path silently bypasses every new safety check. | +120 |
...
| `packages/meta/cli/src/__tests__/start-nexus-wiring.test.ts` | New: headless `koi start` with `KOI_NEXUS_PERMISSIONS=true + KOI_NEXUS_BOOT_MODE=assert-remote-policy-loaded-at-boot` + healthy HTTP transport boots; same with unreachable transport throws (proves headless honors the gate); `KOI_NEXUS_AUDIT_POISON=true + KOI_NEXUS_AUDIT=true` headless propagates to runtime config; legacy headless deployments without any `KOI_NEXUS_*` vars still boot with current behavior (regression guard); **silent-bypass-guard end-to-end (Loop 5 R7): headless with `KOI_NEXUS_BOOT_MODE=assert-*` but NO consumer flag THROWS at parse time before createKoiRuntime is called; the runtime never receives a transport AND the operator gets a fail-fast error rather than a silent boot. Symmetry test: same input through tui-command.ts also throws.** | +160 |
| `packages/meta/cli/src/__tests__/nexus-config.test.ts` | New: tri-state parsing for every flag; mutual-exclusion enforcement for `--nexus-*` / `--no-nexus-*`; `KOI_NEXUS_BOOT_MODE` rejects unknown values with actionable message; legacy `require` audit-mode rejected; numeric fields parse + bound-check; **silent-bypass guard tests (Loop 5 R7): each control-plane flag set in isolation without consumer flag THROWS naming the offending flag and resolution paths; combinations (multiple control-plane flags) name ALL offenders in one message; with consumer flag enabled, control-plane flags pass through normally; legacy headless deployment with NO Nexus flags continues to boot unchanged** | +160 |
...
**Default-preserving rule (Loop 4 R9, applies SYMMETRICALLY to TUI and headless):** `nexusTransport` is constructed and threaded into `createKoiRuntime` ONLY when the operator explicitly sets `nexusPermissionsEnabled=true` OR `nexusAuditEnabled=true`. This applies to BOTH `tui-command.ts` (interactive) and `commands/start.ts` (headless) — there is no host-driven asymmetry. The fs-nexus filesystem layer (which `koi start` already uses for Nexus-backed manifests) continues to issue remote fs RPCs through its own HTTP/local-bridge transport regardless; this rule governs only the SECOND-class Nexus consumers (centralized permissions, Nexus audit sink) that this PR introduces. The same Nexus-backed manifest gets the SAME security behavior on both hosts given the same `KOI_NEXUS_*` flags.

**Silent-bypass guard (Loop 5 R7):** when the operator sets ANY Nexus control-plane flag that requires a wired consumer to take effect (`nexusBootMode != "telemetry"`, `nexusAuditPoisonOnError === true`, `nexusProbeDeadlineMs`, `nexusBootSyncDeadlineMs`, or a non-default `nexusPolicyPath`) but does NOT set any consumer flag (`nexusPermissionsEnabled` and `nexusAuditEnabled` both unset/false), `parseNexusConfigFromEnvAndFlags` THROWS a config error naming each silent-no-op flag and the resolution paths (`enable a consumer`, OR `clear the control-plane flag if rollback is intended`). Without this guard, an operator who sets `KOI_NEXUS_BOOT_MODE=assert-remote-policy-loaded-at-boot` expecting fail-fast startup would get a silent boot with no probe and no enforcement — exactly the trust-boundary hole that defeats the purpose of the assert-* gate. The guard runs in BOTH tui-command.ts and commands/start.ts (shared parser), preserving symmetry.
