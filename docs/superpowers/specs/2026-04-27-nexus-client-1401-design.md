...
7h_unc10b. `EXPLICIT opt-out + assert-* + NO nexusTransport supplied: THROWS at runtime Step -1 silent-bypass guard. Even though no transport means no Nexus block to bypass, an operator who set both opt-out flags AND assert-* AND zero transport is in an internally inconsistent state — fail-fast surfaces the misconfiguration so the operator clears one or the other rather than carrying stale assert-* config indefinitely. Error names the offending flag and resolution paths (clear assert-* OR enable a consumer + supply transport). Loop 6 R3 — aligns test with runtime pseudocode (the Step -1 guard fires regardless of transport presence; transport-presence is checked separately in the HTTP-implicit-consumer carve-out).`
