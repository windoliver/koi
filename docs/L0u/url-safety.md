...
When the caller passes a `Request` object without overriding `init.headers`, the wrapper passes that Request through to the underlying `fetch` on the first hop so internal state (undici dispatcher on symbols, credentials mode, referrer policy, etc.) survives. Subsequent redirect hops reconstruct from URL and lose any such internal state — that's intentional, since redirects are a new request context. Callers who rely on internal Request state across redirects should follow redirects manually via `redirect: "manual"`.
