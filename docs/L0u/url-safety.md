...
**Transport options are preserved.** `dispatcher` (undici) and `agent` (older Node fetch) flow through unchanged, so callers that require a proxy / custom CA / mTLS / audited egress path keep those guarantees.
