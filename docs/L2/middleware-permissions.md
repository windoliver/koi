...
- **Explicit ask rules cannot be bypassed by user-stored grants** (#2072): `handle-ask-decision.ts` now gates persistent and session always-allow lookups on the `IS_DEFAULT_ASK` marker that `rule-evaluator.ts` stamps on fall-through asks. Explicit asks (rules installed with `effect: "ask"` — e.g. the TUI's "File is outside the workspace — approve to read" rule for `fs_read`) bypass the grant cache and always surface the prompt. Fall-through asks (no rule matched) keep the existing always-allow behavior so users can still remember approvals for unconfigured tools. Closes the S2 bug-bash gap where a `[!] Always` press for an in-workspace `fs_read` silently authorized `/etc/passwd` on every later session.
...
- **Approval steps stamped with per-stream runId** (#1419) — `approval-audit.ts` now writes `step.metadata.runId = ctx.session.runId` on every emitted approval step. Under stable `RuntimeConfig.sessionId` (multiple concurrent streams sharing one sessionId), the runtime's approval-step relay routes by this runId so an approval originating in stream A is delivered to stream A's trajectory document, not broadcast across every concurrent stream's trajectory. No effect when sessionId rotates per stream — the relay falls through to the single-emitter path.
- **Approval-zone interception** (#1644) — optional `zones: { evaluator, sandboxRouter? }` config injects an `@koi/approval-zones` `ZoneEvaluator` into the `ask` path. When the rule evaluator returns `ask` and persistent/session approvals miss, the zone evaluator runs before the user prompt. Verdicts: `auto` skips the prompt and emits a `permission_decision` audit entry with `metadata.permissionEvent = "zone-auto"`; `sandbox` (sandbox-then-auto) runs the bash command in the named backend with a deny-by-default `SandboxProfile` whose `allowRead` is derived from the extracted command paths, then auto-allows the host re-execution; backend-id mismatch or non-bash sandbox verdicts fall through to prompt. For bash, the resource passed to the evaluator is the first extracted path; `context.extraPaths` carries the remaining paths so a zone like `paths: ["/tmp/**"]` rejects `ls /tmp/ok /etc/passwd` instead of allowing it via the first match alone.
