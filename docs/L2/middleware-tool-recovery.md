...
## Recent durability hardening (rounds 40-46)

- **Round 40 F1** — Synthesized `done` after stream-error recovery preserves the originating provider model name and any usage observed on the upstream stream so downstream cost / accounting middleware records recovered turns correctly.
- **Round 42 F2** — New `defaultModel` config option supplies provider identity when `ModelRequest.model` is omitted by the runtime; resolution order is `request.model` → `defaultModel` → `"unknown"`.
