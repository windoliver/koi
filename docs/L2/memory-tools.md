...
During progressive-mode session resets, `liveSkillComponents` is rebuilt from a
fresh `skills-runtime` catalog. Memory's `SkillComponent` is a **body-backed root
skill** — it has `content` set and neither `runtimeBacked` nor `mcpBacked` is true.
The reset merge logic preserves such skills: it seeds from the refreshed
skills-runtime catalog, then re-overlays any component where `!runtimeBacked && !mcpBacked`
(i.e. injected by providers other than skills-runtime, like memory-tools or tool-browser).
This ensures the memory skill guidance survives across session resets without
requiring memory-tools to be re-initialized.
