...
- `VerifyOptions` —
  ...
  - `stageTimeoutMs?: number` — optional per-stage hard timeout for
    COOPERATIVE ASYNC stages. When an awaitable stage's `run` does not
    resolve within the window, the pipeline returns TIMEOUT to the
    caller even if the stage's promise has not settled. The underlying
    work may continue (we cannot kill a Promise), but uncooperative
    plugins that ignore `ctx.signal` cannot wedge the pipeline.
    **Does NOT preempt synchronous / CPU-bound stage code** — stages
    that block the event loop cannot be interrupted from the same
    thread; run untrusted CPU-bound stages in a worker/isolate/process
    that can be preempted at the OS level. Defaults to `undefined`.
