...
| Cache key | The user-supplied `cacheKey` is composed with a fingerprint of the stage list (`name@version` for each stage, joined by `\|`) before any cache call. Adding, removing, renaming, or version-bumping a stage invalidates prior cache entries automatically — a cache hit is only valid for the exact stage configuration that produced it. Bump `VerifierStage.version` whenever the stage's check semantics change in a way callers should re-verify. |
| Result immutability | Every returned `ForgeVerificationSummary` is `Object.freeze`d with frozen `stageResults` digests. The in-memory cache also deep-copies on `set`. Caller mutation of a returned summary cannot poison the cache, and a stage that retains a reference to a prior digest cannot rewrite the trail. |
