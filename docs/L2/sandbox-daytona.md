...
- `destroy()` — permanently delete the remote workspace. **Requires** `sdk.delete()`. The adapter validates this at `create()` time: if the SDK handle returned by `client.createSandbox()` has no `delete()`, the adapter best-effort-closes the just-provisioned workspace and rejects fail-closed before any caller can use it. Some Daytona SDK versions implement `close()` as a client-side detach that leaves the workspace running and billable; falling back to `close()` later would mark the instance destroyed locally while the remote workspace silently kept running, so we refuse the lifecycle gap up front. The `sdk.delete()` call is bounded at 10 s — on timeout the instance transitions to quarantine, surfaces a leak warning, and `destroy()` remains retryable. Idempotent on success; concurrent calls coalesce.
