...
There is **no implicit default output cap**. Memory bounding only works when the SDK enforces a cap server-side; a post-hoc trim of an already-buffered payload would not actually bound memory or bandwidth, so the adapter refuses to claim a guarantee it cannot keep. To get a cap you must (a) pass `maxOutputBytes` in `SandboxExecOptions` AND (b) inject an SDK wrapper that advertises `commands.supportsMaxOutputBytes=true`. When both conditions are met the cap is forwarded server-side and applied as a **single byte budget across stdout + stderr** (UTF-8-boundary-accurate, with `truncated=true` set on drop). Without an explicit cap the SDK's payload is returned verbatim.
