...
> **Turn index in model step metadata (PR #1758):** `buildModelStep` now accepts an optional `turnIndex` parameter (from `TurnContext.turnIndex`) and includes it in the step's `metadata.turnIndex`. This lets downstream consumers (e.g., `/trajectory` TUI view) detect sub-turn boundaries within a single `run()` call (e.g., multi-turn agent loops with tool use). For interactive TUI sessions where each user message is a separate `run()` call (and `ctx.turnIndex` resets to 0), the TUI injects a synthetic `koi:tui_turn_start` boundary step instead — see `docs/L3/cli.md`.

> **OTel observer hook + ATIF trace stamping (PR #1628):** `EventTraceConfig` gains `onStep?: (sessionId, step) => JsonObject | void`. The callback fires synchronously in `recordStep()` after the `RichTrajectoryStep` is fully assembled but before the fire-and-forget store write. The return value is shallow-merged into `step.metadata` before the write — giving OTel accurate retroactive timing and allowing both systems to share trace identity. `@koi/middleware-otel` returns `{ "otel.traceId": string, "otel.spanId": string }` from its `onStep` so ATIF steps carry OTel coordinates. Errors in the callback are swallowed (observer-never-throws invariant).

> **Biome formatting pass (#1636):** No behavioral changes — auto-formatted by biome check --write.

> **Defense-in-depth secret redaction (PR #2099):** `captureError()` now runs `error.message` and `error.cause` through `@koi/redaction`'s `createRedactor()` before persisting. The formatter middleware sanitizes errors before they reach the model, but event-trace runs in the observe phase — innermost in the onion — so it sees the raw throw before any resolve-phase sanitizer. Applying redaction at the error-capture site closes the gap so ATIF trajectories and audit sinks reading them cannot leak credentials embedded in tool throws. Fail-closed: redactor failures fall back to `[REDACTION_FAILED]`.
