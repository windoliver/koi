...
- **`@koi/settings` bridge (#1958)**: new `settings-bridge.ts` module translates `KoiSettings` (from `@koi/settings`) into `SourcedRule[]` for direct use by the permission backend. `mapSettingsToSourcedRules(settings, source)` parses each allow/deny/ask string in `settings.permissions` into one or two `SourcedRule` entries using glob encoding (`"Bash(git *)"` → `{ pattern: "Bash:git *", action: "invoke" }`; bare tool name → two rules: exact id + `toolName:**` to prevent prefix bleeding). `widenCommandScopedRulesForTui()` drops command-scoped allow rules (strips overly specific constraints safe to widen for TUI) and widens command-scoped deny/ask patterns to tool-level, matching TUI's single-key permission mode. `TuiRuleNormalizationResult` type exported for callers that need the widening output shape. `CompiledRule` type re-exported from the public index.
- **`RuleSource` extended with `"flag"` tier (#1958)**: `RuleSource` is now `"policy" | "flag" | "project" | "local" | "user"`. Updated precedence: **policy > flag > local > project > user** — the `flag` tier sits between policy and local, representing CLI `--settings` flag overrides. `SOURCE_PRECEDENCE` array updated accordingly.
