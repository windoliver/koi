...
> **Known limitation: stale invalidation events.** `StoreChangeEvent` carries no generation/version field, so a delayed `updated`/`removed`/`quarantined` event for a *prior* generation of a brickId is indistinguishable from an event for the current registration. After a broken executor is re-promoted with a fixed policy, a late event for the old generation will evict the fresh entry — turning subsequent calls into cache misses that fall through to the unwrapped tool path. Mitigation requires an L0 change to add a generation/epoch token to `StoreChangeEvent` so the cache can ignore stale events. Tracked as a follow-up; the failure mode is fail-misses (deny silently reverts to tool-runs-normally) rather than fail-allow on every call, but it is real and should be closed before this middleware is relied on for hard authorization. Hosts wiring this middleware against high-churn forge promotions should currently issue an idempotent `evict()` followed by a fresh `register()` rather than relying on the notifier.
