...
| `priority` | `number` | `800` (`DEFAULT_PRIORITY`) | Forwarded to `RlmConfig.priority`. The stack rejects values below `DEFAULT_PRIORITY` as a **best-effort floor** — it blocks the obvious footgun where RLM would segment before the v1 tool-injectors known at floor-design time (e.g. `tool-selector` at 200). It does **not** prove the full ordering invariant: composers stacking custom tool-injecting `wrapModelCall` middleware above 800 must additionally verify their relative ordering or bump RLM's priority above those peers. Use `createRlmMiddleware` directly to opt out. |
