...
| `@koi/scheduler` | L2 | In-process task scheduler — delayed dispatch, cron schedules, retry/backoff, dead-letter. SQLite-backed stores (`createSqliteTaskStore`, `createSqliteScheduleStore`, `createSqliteRunStore`). Wired in the TUI via the `schedulerStack` preset stack with an in-memory SQLite database, no-op dispatcher, and agentId pinned from host context. (#1384) |
