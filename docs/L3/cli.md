...
| `@koi/middleware-extraction` | L2 | Post-turn learning extraction — intercepts spawn-family tool outputs, extracts reusable knowledge via regex + LLM, persists to in-memory memory backend. **Fix (#1964):** `heuristic`/`pattern` → `feedback` (was `reference`). |
