...
- **`koi tui` artifact tools (#1651, Plans 1+2)**: the TUI opens a single `@koi/artifacts` `ArtifactStore` per process at `~/.koi/artifacts/` (sqlite meta + `@koi/blob-cas` blob dir) and attaches four agent-facing tools (`artifact_save`, `artifact_get`, `artifact_list`, `artifact_delete`) via a new `extraProviders` hook on `createKoiRuntime`. All tool calls bind to the active TUI session id; advisory-lock contention (concurrent TUIs hitting the same path) logs `artifact store disabled — ArtifactStore already open by another process` to stderr and continues without artifact tools rather than aborting the session. Both interim + final teardown paths close the store so the lock releases cleanly on normal quit, SIGUSR1, or dispose failure.
