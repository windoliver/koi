...
> **Plugin schema strict + rejected-manifest surfacing:** `@koi/plugins` schema is `.strict()` so unknown top-level fields (e.g. the `mcp_servers` vs `mcpServers` typo) are rejected at load time instead of being silently stripped. `listPlugins` now returns `{ entries, errors }` so `koi plugin list` can print rejected manifests below the successful-plugin table — each line shows the directory path, source tier, error code, and error message. Text-mode `koi plugin list` exits `ExitCode.FAILURE` when any plugin was rejected so shell scripts can detect problems without parsing stdout; JSON mode (`--json`) emits `{ plugins: [...], errors: [...] }` and also exits non-zero on rejection. A user whose plugin silently wasn't loading now sees the exact validation error at the seam where they were already looking. See `docs/L2/plugins.md` for the strict-schema details.
