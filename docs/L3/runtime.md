...
| `@koi/governance-core` | Policy gate + setpoint enforcement middleware (#1392). Fail-closed `gate()` runs controller `checkAll()` + `backend.evaluator.evaluate()` before every model/tool call; setpoint denies throw `RATE_LIMIT`, policy denies throw `PERMISSION`, and both emit compliance records. Post-call recording awaits controller completion so the next gate reads up-to-date state; accounting failures latch a degraded flag that denies subsequent calls until the middleware is rebuilt. Trust-boundary hardening: `costUsd` validated for `Number.isFinite && >= 0`, token counts validated the same way, cost-calc failure omits `costUsd` so controller per-token fallback pricing runs. Turn-count recording in `onAfterTurn` (defense-in-depth: throws AND latches to survive host `.catch(noop)` wrappers). Stream accounting accumulates incremental usage deltas, honors `error.usage` as authoritative, and terminal recording runs before yielding the terminal chunk so accounting errors surface on the normal control path. Tool outcome recording (`tool_success`/`tool_error`) drives the controller's `error_rate` variable. Compliance redaction strips tool-call input values (keeps `toolId` + `inputKeys`). | `governance-allow`, `governance-deny` |
