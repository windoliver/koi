...
| `@koi/tools-bash` | Bash execution tool: cwd-contained, env-isolated, process-group kill, exfiltration denylist. Includes `createBashBackgroundTool()` for fire-and-forget subprocess execution via `ManagedTaskBoard`, shared `exec.ts` spawn/drain layer, `trackCwd` sentinel for directory tracking, OS sandbox DI (`sandboxAdapter` + `sandboxProfile`), and `SpawnTransform` argv-based injection hook | `bash-exec`, `bash-background`, `bash-track-cwd` |
