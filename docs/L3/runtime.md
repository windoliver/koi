...
| `@koi/memory-tools` | Memory read/write/list tools — sandboxed with `memoryDir` filesystem caps, atomic `storeWithDedup`, idempotent delete, SkillComponent behavioral guidance | `memory-store` |
| `@koi/middleware-memory-recall` | Frozen-snapshot memory recall — scans `.koi/memory/` once per session, scores by salience, budgets to 8K tokens, injects into every model call. Optional per-turn relevance selector (model-agnostic). Per-session state isolation | `memory-recall` |
...
| `@koi/tools-bash` | Bash execution tool: cwd-contained, env-isolated, process-group kill, exfiltration denylist. Now imports `classifyBashCommand` + `initializeBashAst` from `@koi/bash-ast` (PR #1660) instead of `@koi/bash-security` — both `createBashTool` and `createBashBackgroundTool` await `initializeBashAst()` at the top of `execute()` before the sync classifier reads the cached parser. Also includes `createBashBackgroundTool()` for fire-and-forget subprocess execution via `ManagedTaskBoard`, shared `exec.ts` spawn/drain layer, `trackCwd` sentinel for directory tracking, OS sandbox DI (`sandboxAdapter` + `sandboxProfile`), and `sandboxed` boolean on `BashSuccessResult` indicating OS sandbox confinement. Classifier now includes a `destructive` threat category (#1721) that fires inside `execute()` after the permission modal — catches `rm -rf` on system paths, `mkfs*`, `dd of=/dev/*`, fork bomb, `chmod -R 777 /`, `shutdown`/`reboot`/`halt`/`poweroff`, `init 0/6`; workspace-scoped ops pass through | `bash-exec`, `bash-background`, `bash-track-cwd`, `bash-ast-too-complex` |
