...
> **L0u packages also wired:** `@koi/tools-core` (`buildTool()` factory), `@koi/validation`, `@koi/task-board`, `@koi/rules-loader` (hierarchical project rules file injection — discovers CLAUDE.md/AGENTS.md/.koi/context.md from cwd to git root, merges root-first into system prompt), `@koi/context-manager` (`enforceBudget()` — token-aware transcript compaction with micro/full cascade; `resolveConfig()` + `budgetConfigFromResolved()` for per-model window resolution; golden tests in `golden-queries.test.ts` cover micro-compaction and model-registry window lookup, #1623) are L0u (utility) packages depended on by `@koi/runtime` but not subject to the L2 doc/golden-query gates — their docs live under `docs/L0u/` or `docs/L2/`.
