...
> **PROPOSAL (issue-1377, PR 1 of 5) — this L3 doc remains the authoritative shipped contract for v1.** A successor design for the edge adapters (Cloudflare, Vercel) lives at `docs/superpowers/specs/2026-05-05-edge-sandboxes-design.md`; that spec is a PROPOSAL only. The matching `EdgeFunctionAdapter` types, `koi.edge.cloudflare` runtime accessor, and substantive L3 rewrite land in PR 2 atomically. **Until PR 2 merges, the Cloudflare/Vercel sections below remain the authoritative shipped contract** — neither package exists on `main` today, so what the L3 doc describes is the forward-looking-but-currently-shipped design surface. The proposal at `docs/superpowers/specs/...` describes a future replacement contract that supersedes those sections only when PR 2 merges. This avoids a no-authority interim state.
