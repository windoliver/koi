...
> **CLOUDFLARE / VERCEL EDGE SECTIONS BELOW: NOT CURRENTLY SHIPPED (issue-1377).** Neither `@koi/sandbox-cloudflare` nor `@koi/sandbox-vercel` exists on `main` today. The Cloudflare/Vercel rows in `createCloudSandbox` and the lazy-loaded factory table below are forward-looking placeholder text from a prior planning pass — they describe NO currently-shipped behavior and MUST NOT be cited as a contract for any consumer or implementer. The new design at `docs/superpowers/specs/2026-05-05-edge-sandboxes-design.md` (introduced by this PR) describes the future edge-adapter contract; it lands as code+types in PR 2 of issue-1377, after which this L3 doc is rewritten substantively. **During the interim period after this PR merges:** edge adapters have no authoritative L3 contract because they do not exist as code yet. The wasm executor and OS/SSH adapter sections of this L3 doc remain accurate for what IS shipped on `main`.
