...
import { selfTestError } from "@koi/forge-types";
...
export async function verifySelfTest(
  ...
  testCounts?: { readonly agentProvided: number; readonly autoGenerated: number },
  ...
  const testCasesRun =
    hasTestCases && "testCases" in input && input.testCases !== undefined
  ...
  return {
    ...
    value: {
      ...
      durationMs,
      testCasesRun,
      agentProvidedCount: testCounts?.agentProvided ?? testCasesRun,
      autoGeneratedCount: testCounts?.autoGenerated ?? 0,
    },
  };
}
