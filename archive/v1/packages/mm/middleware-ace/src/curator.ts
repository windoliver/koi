...
function buildCuratorPrompt(input: CuratorInput): string {
  ...
  const bulletCount = playbook.sections.reduce((sum, s) => sum + s.bullets.length, 0);

  return [
    "You are a master curator of an AI agent's operational playbook.",
    "",
    "**Your job:** Update the playbook with NEW insights from a session reflection.",
    "The playbook helps the agent make better tool usage decisions in future sessions.",
    "",
    "**Current playbook" +
      (bulletCount > 0 ? ` (${String(bulletCount)} bullets)` : " (empty)") +
      ":**",
    playbookSummary || "(No bullets yet)",
    "",
    "**Reflection from latest session:**",
    `Root cause analysis: ${reflection.rootCause}`,
    `Key insight: ${reflection.keyInsight}`,
    "",
    `Token budget: ${String(tokenBudget)} tokens (current playbook must fit within this)`,
    "",
    "**Produce a JSON array of delta operations:**",
    '- { "kind": "add", "section": "<slug>", "content": "<new bullet>" } — add a NEW insight',
    '- { "kind": "merge", "bulletIds": ["<id1>", "<id2>"], "content": "<merged>" } — combine redundant bullets',
    '- { "kind": "prune", "bulletId": "<id>" } — remove a bullet that is consistently harmful',
    "",
    "**Section slugs:** " + playbook.sections.map((s) => `"${s.slug}" (${s.name})`).join(", "),
    "",
    "**Critical rules:**",
    "- Only add bullets that are SPECIFIC and ACTIONABLE (e.g., 'when fs_read returns error X, try Y')",
    "- Do NOT add generic advice (e.g., 'implement robust error handling')",
    "- Do NOT add bullets about middleware behavior the agent cannot control",
    "- Avoid redundancy — if similar advice exists, merge instead of adding",
    "- Each bullet should describe a concrete tool usage pattern or error recovery strategy",
    "- Prune bullets where harmful > helpful (they're hurting agent performance)",
    "- If the reflection shows no new actionable insight, return an empty array []",
    "",
