...
function createMockSdk(overrides?: Partial<VercelSdkSandbox>): VercelSdkSandbox {
  return {
    commands: {
...
describe("createVercelInstance", () => {
  test("exec runs command and returns result", async () => {
    const sdk = createMockSdk();
    const instance = createVercelInstance(sdk);
  ...
  test("throws after destroy", async () => {
    ...
    const instance = createVercelInstance(sdk);
    await instance.destroy();
    expect(() => instance.exec("cmd", [])).toThrow("vercel: cannot call exec() after destroy()");
  });
