...
// ---------------------------------------------------------------------------
// Entry-path execution (dependency-backed bricks with import-based entrypoints)
// ---------------------------------------------------------------------------

/**
 * Execute a brick via its workspace entry file using dynamic import().
 *
 * Used for dependency-backed bricks that require npm packages and cannot
 * run through new Function(). The entry file is generated by the forge
 * dependency resolver and lives inside the brick's workspace directory.
 *
 * @param entryPath - Absolute path to the brick's entry .ts file.
 * @param input - Input data passed to the brick's default export.
 * @param timeoutMs - Execution timeout.
 * @param workspacePath - Optional workspace directory to chdir into before import.
 * @param onTimeout - Injectable timeout action for testing.
 */
export async function executeEntry(
  ...
  try {
    ...
    if (typeof fn !== "function") {
      ...
      return formatError(
        "CRASH",
        `Entry module at ${entryPath} does not export a callable default`,
