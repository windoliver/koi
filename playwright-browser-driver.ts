...
export function createPlaywrightBrowserDriver(config: PlaywrightDriverConfig = {}): BrowserDriver {
  ...
  // True only when we called newContext() ourselves — false when CDP reused contexts[0].
  // Only close a context we created; closing a caller-owned CDP context destroys unrelated pages.
  let ownsContext = false; // intentional mutation: set once in ensureContext
    ...
    if (!contextInitPromise) {
      contextInitPromise = (async (): Promise<BrowserContext> => {
        ...
        if (config.userDataDir && !config.browser && !config.cdpEndpoint) {
          ...
          ownsContext = true; // intentional mutation: we launched this persistent context
        } else {
          const b = await ensureBrowser();
          // For CDP connections, reuse the default context if one exists.
          // We do NOT own a reused CDP context — dispose() must not close it.
          if (config.cdpEndpoint) {
            const contexts = b.contexts();
            if (contexts[0]) {
              ctx = contexts[0]; // reused — caller owns its lifecycle
            } else {
              ctx = await b.newContext();
              ownsContext = true; // intentional mutation: we created this context
            }
          } else {
            ctx = await b.newContext();
            ownsContext = true; // intentional mutation: we created this context
  ...
  return {
    ...
    async click(ref: string, options?: BrowserActionOptions): Promise<Result<void, KoiError>> {
      try {
        ...
        // Click can trigger navigation or JS that re-renders the page, so the
        // old snapshot refs are stale — force callers to re-snapshot.
        const tabIdAfterClick = getActiveTabId();
        if (tabIdAfterClick) invalidateTabSnapshot(tabIdAfterClick);
    ...
    async type(
      ...
      try {
        ...
        const tabIdAfterType = getActiveTabId();
        if (tabIdAfterType) invalidateTabSnapshot(tabIdAfterType);
        return { ok: true, value: undefined };
      } catch (e: unknown) {
    ...
    async select(
      ...
      try {
        ...
        const tabIdAfterSelect = getActiveTabId();
        if (tabIdAfterSelect) invalidateTabSnapshot(tabIdAfterSelect);
        return { ok: true, value: undefined };
      } catch (e: unknown) {
    ...
    async fillForm(
      ...
      try {
        ...
        // Pass 1: resolve all refs before touching any field — fails fast if any ref is missing.
        // Note: the writes in pass 2 are NOT rolled back on error; if a fill() throws mid-way,
        // earlier fields are already mutated. Callers that need strict atomicity should
        // snapshot → verify → fillForm in a single flow and handle partial-fill errors.
        ...
        const tabIdAfterFill = getActiveTabId();
        if (tabIdAfterFill) invalidateTabSnapshot(tabIdAfterFill);
        return { ok: true, value: undefined };
      } catch (e: unknown) {
    ...
    async dispose(): Promise<void> {
      ...
      // Only close context if we created it — reused CDP contexts are caller-owned.
      if (ownsContext && browserContext) {
