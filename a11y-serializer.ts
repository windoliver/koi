...
// Sanitize page-derived strings before embedding in the snapshot format.
// The output format uses `"name"` for accessible names and `[attr="val"]`
// for state attrs, so quotes and bracket chars must be neutralised to prevent
// a page from injecting fake syntax or breaking the format parser.
function sanitizeName(s: string): string {
  return s.replace(/[\r\n]/g, " ").replace(/"/g, "'");
}
function sanitizeAttrValue(s: string): string {
...
export function serializeA11yTree(
  ...
  function visit(node: A11yNode, depth: number): boolean {
    ...
    if (node.name) {
      line += ` "${sanitizeName(node.name)}"`;
    }

    // Collect state attributes
    const attrs: string[] = [];
    if (node.level !== undefined) attrs.push(`level=${node.level}`);
    if (node.value !== undefined) attrs.push(`value="${sanitizeAttrValue(String(node.value))}"`);
    ...
    if (node.description) attrs.push(`desc="${sanitizeAttrValue(node.description)}"`);
...
export function parseAriaYaml(yaml: string, options?: BrowserSnapshotOptions): SerializeResult {
  ...
  for (const rawLine of yaml.split("\n")) {
    ...
    // Append inline YAML text (e.g. `- paragraph: Some text.`) so agents
    // can see non-interactive page content like paragraphs and error messages.
    const inlineText = m[5]?.trim();
    if (inlineText) outLine += `: ${sanitizeAttrValue(inlineText)}`;
