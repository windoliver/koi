...
export function translatePlaywrightError(operation: string, err: unknown): KoiError {
  ...
  // The bare "SyntaxError: " check is intentionally gated behind selector-specific
  // phrases to avoid misclassifying unrelated JS evaluation SyntaxErrors (e.g. from
  // page script eval) as user-fixable selector problems.
  const isSelectorError =
    msgIncludes(msg, "is not a valid selector", "Failed to execute 'querySelector'", "Invalid selector") ||
    (msg.includes("SyntaxError: ") &&
      msgIncludes(msg, "selector", "querySelector", "css", "xpath", "locator"));
  if (isSelectorError) {
